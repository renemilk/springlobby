...
public:
  ...
  wxKeyBind(int flags, int keycode, size_t orderIndex = 0) { Set(flags, keycode, orderIndex); }

  wxKeyBind(const wxKeyBind& tocopy) { DeepCopy(tocopy); }

  wxKeyBind(const wxString& key, size_t orderIndex = 0) { Set(key, orderIndex); }
  ...
  virtual bool operator!=(const wxKeyBind& other) const { return !((*this) == other); }
...
public:
  ...
  void Set(const wxString& key, size_t orderIndex) {
    Set(StringToKeyModifier(key), StringToKeyCode(wxKeyBind::discardModifier(key)), //.AfterLast('+')).AfterLast('-'));,
        orderIndex);
  }
  ...
  void Set(const wxKeyBind& key) { Set(key.m_nFlags, key.m_nKeyCode, key.m_nOrderIndex); }
  ...
  // Getters
  // ------------------
  size_t GetOrderIndex() const { return m_nOrderIndex; }

  void SetOrderIndex(size_t orderIndex) { m_nOrderIndex = orderIndex; }

  int GetKeyCode() const { return m_nKeyCode; }

  int GetModifiers() const { return m_nFlags; }

  bool HasAnyModifier() const { return (m_nFlags & wxACCEL_ANY) != 0; }

  // Returns the string which describes this key combination.
  wxString GetStr() const { return KeyModifierToString(m_nFlags) + KeyCodeToString(m_nKeyCode); }

  wxAcceleratorEntry GetAccelerator(int cmdId) const { return wxAcceleratorEntry(GetModifiers(), GetKeyCode(), cmdId); }
...
public:
  ...
  virtual bool operator!=(const wxCmd& other) const { return !((*this) == other); }
...
public:
  ...
  wxAcceleratorEntry GetAccelerator(int n) const { return GetShortcut(n)->GetAccelerator(m_nId); }

  int GetId() const { return m_nId; }

  int GetShortcutCount() const { return m_nShortcuts; }
...
public:
  ...
  virtual ~wxBinderEvtHandler() { m_pTarget->RemoveEventHandler(this); }
  ...
  wxKeyBinder* GetBinder() const { return m_pBinder; }
...
public: // miscellaneous
  ...
  // Add functions
  // -------------------
  void SetMetaKey(const wxString& key) { this->m_metaKey = key; }

  const wxString& GetMetaKey() const { return this->m_metaKey; }

  void SetKeySyms(const key_sym_map& keySyms) { this->m_keySyms = keySyms; }

  void SetKeySymsSet(const key_sym_set_map& keySymsSet) { this->m_keySymsSet = keySymsSet; }

  const key_sym_map& GetKeySyms() const { return this->m_keySyms; }

  const key_sym_set_map& GetKeySymsSet() const { return this->m_keySymsSet; }

  void AddCmd(wxCmd* p) { m_arrCmd.Add(p); }
  ...
  // Getters
  // -------------------
  int GetCmdCount() const { return m_arrCmd.GetCount(); }

  int GetAttachedWndCount() const { return m_arrHandlers.GetCount(); }

  wxWindow* GetWindow(int n) const { return ((wxBinderEvtHandler*)m_arrHandlers.Item(n))->GetTargetWnd(); }
...
public:
  wxKeyProfile(const wxString& name = wxEmptyString, const wxString& desc = wxEmptyString, bool notDeletable = false,
  ...
  wxKeyProfile(const wxKeyProfile& tocopy) : wxKeyBinder() { DeepCopy(tocopy); }

  wxKeyProfile(const wxKeyProfile& tocopy, bool notDeletable, bool notEditable) {
    DeepCopy(tocopy);
    m_notDeletable = notDeletable;
    m_notEditable = notEditable;
  }
...
public:
  ...
  wxKeyProfile* GetSelProfile() {
    wxASSERT(m_nSelected >= 0 && m_nSelected < static_cast<int>(GetCount()));
...
#define wxKEYBINDER_DEFAULT_STYLE                                                                                      \
  (wxKEYBINDER_SHOW_APPLYBUTTON | wxKEYBINDER_USE_TREECTRL | wxKEYBINDER_SHOW_ADDREMOVE_PROFILE |                      \
   wxKEYBINDER_ENABLE_PROFILE_EDITING | wxTAB_TRAVERSAL) /* wxTAB_TRAVERSAL is the default style for a wxPanel */
...
public:
  ...
  wxKeyConfigPanel(wxWindow* parent, wxWindowID id = -1, const wxPoint& pos = wxDefaultPosition,
                   const wxSize& size = wxDefaultSize, long style = wxKEYBINDER_DEFAULT_STYLE,
                   const wxString& name = wxT("wxKeyConfigPanel"), const wxString& customButton1Label = wxT(""),
                   const wxObjectEventFunction customButton1Event = NULL, const wxString& customButton2Label = wxT(""),
                   const wxObjectEventFunction customButton2Event = NULL, const wxString& customLabelText = wxT("")) {

    Create(parent, id, pos, size, style, name, customButton1Label, customButton1Event, customButton2Label,
           customButton2Event, customLabelText);
  }
...
public: // output-access utilities (to call AFTER ShowModal)
  ...
  wxKeyProfile* GetProfile(int n) const {
    ...
    return (wxKeyProfile*)static_cast<wxItemContainer*>(m_pKeyProfiles)->GetClientData(n);
  }
...
public: // miscellaneous
  ...
  void DisableKeyProfiles() { EnableKeyProfiles(FALSE); }
...
protected: // utilities
  ...
  bool IsUsingTreeCtrl() const { return HasFlag(wxKEYBINDER_USE_TREECTRL); }
