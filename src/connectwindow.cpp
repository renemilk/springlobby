...
ConnectWindow::ConnectWindow(wxWindow* parent, Ui& ui)
  ...
  m_server_combo->SetToolTip(
      TE(_("Server to connect to. You can connect to any server you like by typing in hostaddress:port format.")));
  ...
  m_autoconnect_check->SetToolTip(
      TE(_("remember connection details and automatically connect to server on next lobby startup")));
  ...
  m_note_lbl = new wxStaticText(
      m_login_tab, -1,
      _("Note: If you do not have an account, you\n can register one for free on the\n\"Register\" tab."));
  ...
  m_regpass1_text =
      new wxTextCtrl(m_register_tab, wxID_ANY, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxTE_PASSWORD);
  ...
  m_regpass2_lbl =
      new wxStaticText(m_register_tab, wxID_ANY, wxT("Retype password"), wxDefaultPosition, wxDefaultSize, 0);
...
void ConnectWindow::OnServerChange(wxCommandEvent&) {
  ...
  if (sett().GetServerAccountSavePass(HostAddress))
  ...
  if (m_tabs->GetSelection() <= 0) {
    ...
    if (!IsValidNickname(m_regnick_text->GetValue())) {
      customMessageBox(SL_MAIN_ICON,
                       _("The entered nickname contains invalid characters like )? &%.\n Please try again"),
                       _("Invalid nickname"), wxOK);
      Show();
      ...
      customMessageBox(SL_MAIN_ICON,
                       _("Registration failed, the reason was:\nPassword / confirmation mismatch (or empty passwort)"),
                       _("Registration failed."), wxOK);
...
void ConnectWindow::OnCancel(wxCommandEvent&) { Hide(); }

void ConnectWindow::OnQuit(wxCommandEvent& /*data*/) { EndModal(wxCANCEL); }
