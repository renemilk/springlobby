...
MainWindow::MainWindow()
  ...
  m_func_tabs = new SLNotebook(this, _T("mainfunctabs"), MAIN_TABS, wxDefaultPosition, wxDefaultSize,
                               wxAUI_NB_WINDOWLIST_BUTTON | wxAUI_NB_TAB_SPLIT | wxAUI_NB_TAB_MOVE |
                                   wxAUI_NB_TAB_EXTERNAL_MOVE | wxAUI_NB_SCROLL_BUTTONS | wxAUI_NB_LEFT);
...
void MainWindow::OnMessage(wxCommandEvent& event) { customMessageBoxNoModal(SL_MAIN_ICON, event.GetString()); }
...
bool MainWindow::HasFocus() const { return m_has_focus; }
...
ChatPanel* MainWindow::GetChannelChatPanel(const wxString& channel) { return m_chat_tab->GetChannelChatPanel(channel); }
...
void MainWindow::ShowSingleplayer() { ShowTab(PAGE_SINGLE); }
...
void MainWindow::ShowChannelChooser() {
  ...
  if (!ui().IsConnected())
    customMessageBox(SL_MAIN_ICON, _("You need to be connected to a server to view the channel list"),
                     _("Not connected"));
  else {
...
void MainWindow::OnMenuConnect(wxCommandEvent& /*unused*/) { ui().ShowConnectWindow(); }

void MainWindow::OnMenuDisconnect(wxCommandEvent& /*unused*/) { ui().Disconnect(); }

void MainWindow::OnMenuServerTab(wxCommandEvent& /*unused*/) { ui().ReopenServerTab(); }

void MainWindow::OnMenuSaveOptions(wxCommandEvent& /*unused*/) { sett().SaveSettings(); }

void MainWindow::OnMenuQuit(wxCommandEvent& /*unused*/) { Close(); }

#include "utils/uievents.h"

void MainWindow::OnMenuVersion(wxCommandEvent& /*unused*/) { ui().CheckForUpdates(); }
...
void MainWindow::OnShowScreenshots(wxCommandEvent& /*unused*/) {
  ...
  for (auto i : LSL::usync().GetScreenshotFilenames())
    ar.Add(TowxString(i));
  if (ar.Count() == 0) {
    customMessageBoxNoModal(SL_MAIN_ICON, _("There were no screenshots found in your spring data directory."),
                            _("No files found"));
    return;
  }
  ar.Sort();
...
void MainWindow::OnShowDocs(wxCommandEvent& /*unused*/) { OpenWebBrowser(_T("http://springlobby.info")); }
...
void MainWindow::OnShowChannelChooser(wxCommandEvent& /*unused*/) { ShowChannelChooser(); }
...
void MainWindow::OnChannelListStart() { m_channel_chooser->ClearChannels(); }
...
const MainWindow::TabNames& MainWindow::GetTabNames() { return m_tab_names; }
...
void MainWindow::OnMenuPathInfo(wxCommandEvent& /*event*/) { InfoDialog(this).ShowModal(); }
