...
static inline int ReadInt24(const unsigned char* p) { return p[0] | (p[1] << 8) | (p[2] << 16); }

MapCtrl::MapCtrl(wxWindow* parent, int size, IBattle* battle, bool readonly, bool fixed_size, bool draw_start_types,
                 bool singleplayer)
...
wxRect MapCtrl::GetDrawableRect() const {
  int w(0), h(0);
  GetClientSize(&w, &h);
...
void MapCtrl::DrawStartRect(wxDC& dc, int index, wxRect& sr, const wxColour& col, bool mouseover, int alphalevel,
  ...
  wxColour light;
  light.Set(((col.Red() + 100) > 200) ? 200 : (col.Red() + 100),
            ((col.Green() + 100) > 200) ? 200 : (col.Green() + 100),
            ((col.Blue() + 100) > 200) ? 200 : (col.Blue() + 100));
  ...
  if (index != -1) {
    ...
    // dc.DrawText( wxFormat( _T("%d"), index+1), sr.x + sr.width / 2 - twidth / 2, sr.y + sr.height / 2 - theight / 2 -
    // 1 );

    const double metal = GetStartRectMetalFraction(index);
    if (metal != 0.0) {
      wxString strMetal = wxFormat(_("Metal: %.1f%%")) % (std::isnan(metal) ? 0.0 : metal * 100.0);
...
wxRealPoint MapCtrl::GetUserMapPositionAsReal(const User& user) const {
...
wxPoint MapCtrl::GetTranslatedScaledUserMapPosition(const User& user) const {
  wxRealPoint rmp(GetUserMapPositionAsReal(user));
  wxRect mr(GetMinimapRect());

  return wxPoint(mr.x + static_cast<int>(rmp.x * mr.width), mr.y + static_cast<int>(rmp.y * mr.height));
}
 ...
 */
inline wxPoint FitInside(const wxRect& what, const wxRect& container) {
...
void MapCtrl::DrawOutlinedText(wxDC& dc, const wxString& str, int x, int y, const wxColour& outline,
...
void MapCtrl::DrawUser(wxDC& dc, User& user, bool selected, bool /*unused*/) {
  ...
  if (selected) {
    ...
    DrawOutlinedText(dc, wxFormat(_("ally:   %d")) % (user.BattleStatus().ally + 1), r.x + 3, r.y + updownallyrect.y,
                     wxColour(50, 50, 50), *wxWHITE);
    ...
    dc.SetFont(b);
    DrawOutlinedText(dc, wxFormat(_("bonus: %d%%")) % user.BattleStatus().handicap, r.x + 3,
                     r.y + updownhandicaprect.y + 2, wxColour(50, 50, 50), *wxWHITE);
    dc.SetFont(f);
...
void MapCtrl::DrawUserPositions(wxDC& dc) {
  ...
  for (int i = 0; i < int(m_map.info.positions.size()); i++) {
    ...
    for (unsigned int bi = 0; bi < m_battle->GetNumUsers(); bi++) {
      ...
      if (&tbot == 0)
        continue;
      if ((tbot.BattleStatus().pos.x == m_map.info.positions[i].x) &&
          (tbot.BattleStatus().pos.y == m_map.info.positions[i].y)) {
        bot = &tbot;
        break;
      }
    }
...
void MapCtrl::OnResize(wxSizeEvent& /*unused*/) { UpdateMinimap(); }
...
void MapCtrl::OnMouseMove(wxMouseEvent& event) {
  ...
  if (longval == IBattle::ST_Pick) {
    ...
    if (m_maction == Moved) {
      ...
      user.BattleStatus().pos.y =
          (int)(((double)(event.GetY() - mr.y) / (double)mr.height) * (double)m_map.info.height);
  ...
  if (m_maction == Add) { // We are currently adding a rect.
    ...
    m_tmp_brect = GetBattleRect(m_mdown_x < event.GetX() ? m_mdown_x : event.GetX(),
                                ...
                                m_mdown_y > event.GetY() ? m_mdown_y : event.GetY(), m_tmp_brect.ally);
  ...
  if (GetMinimapRect().Contains(p)) {
    ...
    for (int i = m_battle->GetLastRectIdx(); i >= 0; i--) {
      ...
      if (r.Contains(p)) {
        ...
        if (!m_ro) {
          if ((wxRect(r.x + r.width - m_close_img->GetWidth(), r.y + 1, m_close_img->GetWidth(),
                      m_close_img->GetWidth())).Contains(p))
            m_rect_area = UpRight;
          ...
          // else if ( (wxRect( r.x, r.y + r.height - boxsize, boxsize, boxsize )).Contains( p ) ) m_rect_area =
          // DownLeft;
          else
            m_rect_area = Main;
        }
...
void MapCtrl::OnLeftUp(wxMouseEvent& event) {
  ...
  if (longval == IBattle::ST_Pick) {
    ...
    if ((m_mdown_area == Move) && (m_maction == Moved)) {
      ...
      user.BattleStatus().ally =
          (user.BattleStatus().ally - 1) >= 0 ? (user.BattleStatus().ally - 1) : (SPRING_MAX_ALLIES - 1);
      RefreshRect(GetUserRect(user, true), false);
  ...
  if (m_maction == Add) {
    ...
    if ((abs(m_mdown_x - event.GetX()) >= minboxsize) && (abs(m_mdown_y - event.GetY()) >= minboxsize)) {
      BattleStartRect r = GetBattleRect(
          m_mdown_x < event.GetX() ? m_mdown_x : event.GetX(), m_mdown_y < event.GetY() ? m_mdown_y : event.GetY(),
          m_mdown_x > event.GetX() ? m_mdown_x : event.GetX(), m_mdown_y > event.GetY() ? m_mdown_y : event.GetY());
