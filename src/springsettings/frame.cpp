...
void settings_frame::handleExternExit() {
  if (!alreadyCalled) {
    ...
    if (settingsChangedAbstract()) {
      int choice = customMessageBox(SS_MAIN_ICON, _("Save Spring settings before exiting?"), _("Confirmation needed"),
...
void settings_frame::handleExit() {
  if (settingsChangedAbstract()) {
    int action = customMessageBox(SS_MAIN_ICON, _("Save Spring settings before exiting?"), _("Confirmation needed"),
...
void settings_frame::OnMenuChoice(wxCommandEvent& event) {
  switch (event.GetId()) {
    ...
    case ID_MENUITEM_RESET:
      if ((customMessageBox(SS_MAIN_ICON, _("Reset ALL settings to default values?"), _("Confirmation needed"),
                            wxYES_NO)) == wxYES) {
        resetSettings();
      }
      break;
