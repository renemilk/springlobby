...
BattleListCtrl::~BattleListCtrl() { delete m_popup; }
...
wxString BattleListCtrl::GetItemText(long item, long column) const {
  ...
  switch (column) {
    ...
    case 4:
      return (battle.GetHostMapName());
    case 5:
      return (battle.GetHostModName());
    case 6:
      return (opts.founder);
    case 7:
      return wxFormat(_T("%d")) % int(battle.GetSpectators());
    case 8:
      return wxFormat(_T("%d")) % (int(battle.GetNumUsers()) - int(battle.GetSpectators()));
    case 9:
      return wxFormat(_T("%d")) % int(battle.GetMaxPlayers());
    case 10:
      return (wxTimeSpan(0 /*h*/, 0 /*m*/, battle.GetBattleRunningTime()).Format(_T("%H:%M")));
    case 11:
      return battle.GetEngineVersion();
  }
}
...
int BattleListCtrl::GetItemColumnImage(long item, long column) const {
  ...
  switch (column) {
    ...
    case 4:
      return battle.MapExists() ? icons().ICON_EXISTS : icons().ICON_NEXISTS;
    case 5:
      return battle.ModExists() ? icons().ICON_EXISTS : icons().ICON_NEXISTS;
  }
...
int BattleListCtrl::CompareOneCrit(DataType u1, DataType u2, int col, int dir) const {
  switch (col) {
    ...
    case 3:
      return dir * u1->GetDescription().CmpNoCase(u2->GetDescription());
    case 4:
      return dir * u1->GetHostMapName().CmpNoCase(u2->GetHostMapName());
    case 5:
      return dir * u1->GetHostModName().CmpNoCase(u2->GetHostModName());
    case 6: {
      try {
