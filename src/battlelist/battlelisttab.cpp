...
#if wxUSE_TOGGLEBTN
  m_filter_show =
      new wxToggleButton(this, BATTLE_LIST_FILTER_BUTTON, _(" Filter "), wxDefaultPosition, wxSize(-1, 28), 0);
#else
...
#if wxUSE_TOGGLEBTN
  m_info_show =
      new wxToggleButton(this, BATTLE_LIST_INFO_BUTTON, _(" Battle infos "), wxDefaultPosition, wxSize(-1, 28), 0);
#else
  m_info_show =
      new wxCheckBox(this, BATTLE_LIST_INFO_BUTTON, _(" Battle infos "), wxDefaultPosition, wxSize(-1, 28), 0);
#endif
...
void BattleListTab::SelectBattle(IBattle* battle) {
  ...
  if (m_sel_battle != 0) {
    ...
    m_players_text->SetLabel(wxFormat(_T( "%d / %d" )) %
                             (int(m_sel_battle->GetNumUsers()) - int(m_sel_battle->GetSpectators())) %
                             int(m_sel_battle->GetMaxPlayers()));
...
void BattleListTab::OnHost(wxCommandEvent& /*unused*/) {
  ...
  if (battle != 0) {
    if (ui().Ask(_("Already in a battle"),
                 _("You are already in a battle.\n\nDo you want to leave current battle to start a new?"))) {
      battle->Leave();
...
void BattleListTab::DoJoin(Battle& battle) {
  ...
  if (!ui().IsSpringCompatible(battle.GetBattleOptions().engineName, battle.GetBattleOptions().engineVersion)) {
    ...
    //		customMessageBox( SL_MAIN_ICON, _( "Joining battles is disabled due to the incompatible spring version you're
    // using." ), _( "Spring error" ), wxICON_EXCLAMATION | wxOK );
    return;
  }
  ...
  if (cpus.find(battle.GetFounder().GetCpu()) != cpus.end()) {
    const wxString msg(
        _("You are about to join a matchmaking battleroom.\nThe host might move you to another battle at any time. In "
          "case of problems please contact the host or a lobby moderator.\nDo you want to continue?"));
  ...
  if (curbattle != 0) {
    if (ui().Ask(_("Already in another battle"),
  ...
  if (!battle.ModExists()) {
    if (customMessageBox(SL_MAIN_ICON,
                         _("You need to download the game before you can join this game.\n\n") + downloadProc,
  ...
  if (!battle.MapExists()) {
    if (customMessageBox(SL_MAIN_ICON,
                         _("You need to download the map to be able to play in this game.\n\n") + downloadProc,
...
void BattleListTab::UpdateHighlights() { m_battle_list->RefreshVisibleItems(); }

void BattleListTab::SortBattleList() { m_battle_list->SortList(); }
...
void BattleListTab::OnInfoShow(wxCommandEvent& /*unused*/) { ShowExtendedInfos(m_info_show->GetValue()); }
