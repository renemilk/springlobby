...
inline int MapGridCtrl::CompareTidalStrength(const MapData* a, const MapData* b) { CMP(tidalStrength); }
inline int MapGridCtrl::CompareGravity(const MapData* a, const MapData* b) { CMP(gravity); }
inline int MapGridCtrl::CompareMaxMetal(const MapData* a, const MapData* b) { CMP(maxMetal); }
inline int MapGridCtrl::CompareExtractorRadius(const MapData* a, const MapData* b) { CMP(extractorRadius); }
inline int MapGridCtrl::CompareMinWind(const MapData* a, const MapData* b) { CMP(minWind); }
inline int MapGridCtrl::CompareMaxWind(const MapData* a, const MapData* b) { CMP(maxWind); }
...
inline int MapGridCtrl::ComparePosCount(const MapData* a, const MapData* b) { CMP(positions.size()); }
...
void MapGridCtrl::OnResize(wxSizeEvent&) { CheckInBounds(); }
...
void MapGridCtrl::OnMouseMove(wxMouseEvent& event) {
  ...
  if (m_in_mouse_drag) {
    ...
    if (pos_unscaled.x >= 0 && pos.x < m_size.x && pos_unscaled.y >= 0 && pos.y <= m_size.y &&
        idx < (int)m_grid.size()) {
...
void MapGridCtrl::OnRefresh(wxCommandEvent& /*event*/) { Refresh(); }
