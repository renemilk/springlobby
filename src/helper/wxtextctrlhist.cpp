...
wxTextCtrlHist::wxTextCtrlHist(TextCompletionDatabase& textDb, wxWindow* parent, wxWindowID id, const wxString& value,
                               const wxPoint& pos, const wxSize& size, long /*unused*/)
  ...
  if (keyCode == WXK_UP) {
    ...
    if ((modifier & wxMOD_CONTROL) != 0) {
      ...
      if (regex_currentWord.Matches(selection_Begin_InsertPos)) {
        ...
        // std::cout << "#########: selection_Begin_BeforeCurrentWord: (" <<
        // selection_Begin_BeforeCurrentWord.char_str() << ")" << std::endl;
        ...
        if (hm.size() == 1) {
          ...
          if (hm.size() > 1 && sett().GetCompletionMethod() == Settings::MatchNearest) {
            ...
            bool realCompletion =
                newWord.Len() >= currentWord.Len(); // otherwise we have actually less word than before :P
            if (realCompletion)
              currentWord = newWord;
        ...
        // or
        // if nothing was found remove the typed TAB, so that the User stays comfortable not to remove the TAB by
        // himself.
...
int wxTextCtrlHist::overflow(int i) { return wxTextCtrlBase::overflow(i); }
