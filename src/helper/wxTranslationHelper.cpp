...
wxLocale* wxTranslationHelper::GetLocale() { return m_Locale; }

const wxString& wxTranslationHelper::GetSearchPath() const { return m_SearchPath; }
...
void wxTranslationHelper::GetInstalledLanguages(wxArrayString& names, wxArrayLong& identifiers, int& selected_index) {
  names.Clear();
  identifiers.Clear();
  wxString filename;
  ...
  for (bool cont = dir.GetFirst(&filename, mask, wxDIR_DEFAULT); cont; cont = dir.GetNext(&filename)) {
    ...
    if (langinfo != NULL) {
      wxString mo_file = dir.GetName() + wxFileName::GetPathSeparator() + filename + wxFileName::GetPathSeparator() +
                         _T("LC_MESSAGES") + wxFileName::GetPathSeparator() + m_App.GetAppName().Lower() + wxT(".mo");
...
bool wxTranslationHelper::AskUserForLanguage(wxArrayString& names, wxArrayLong& identifiers, int selected_index) {
  wxCHECK_MSG(names.Count() == identifiers.Count(), false,
              _("Array of language names and identifiers should have the same size."));
  long index = GetSingleChoiceIndex(_("Select the language"), _("Language"), names, selected_index);
  if (index != -1) {
    if (m_Locale) {
      wxDELETE(m_Locale);
    }
