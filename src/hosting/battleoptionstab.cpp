...
BattleOptionsTab::BattleOptionsTab(wxWindow* parent, IBattle* battle)
  ...
  m_allowed_list = new wxListBox(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, 0, NULL,
                                 wxLB_MULTIPLE | wxLB_NEEDED_SB | wxLB_SORT);
  ...
  m_restrict_list = new wxListBox(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, 0, NULL,
                                  wxLB_MULTIPLE | wxLB_NEEDED_SB | wxLB_SORT);
...
void BattleOptionsTab::ReloadRestrictions() {
  ...
  try {
    m_allowed_list->InsertItems(
        LSL::Util::vectorToArrayString(LSL::usync().GetUnitsList(STD_STRING(m_battle->GetHostModName()))), 0);
  }
  catch (...) {
  }
...
bool BattleOptionsTab::IsRestricted(const wxString& name) { return (GetRestrictedUnitIndex(name) >= 0); }
...
void BattleOptionsTab::OnRestrict(wxCommandEvent& /*unused*/) {
  ...
  for (unsigned int i = 0; i < names.Count(); i++) {
    ...
    int count = wxGetNumberFromUser(_("How many units of this type do you wish to allow?"), _T( "" ),
                                    _("Unit restriction"), 0, 0, 500000);
    if (count >= 0)
      m_battle->RestrictUnit(unit, count);
  }
  if (names.Count() > 0)
...
IBattle* BattleOptionsTab::GetBattle() { return m_battle; }
