...
void AutohostHandler::SetBattle(Battle* battle) { m_battle = battle; }
...
void SpringieHandler::Balance() { m_battle->Say(_T("!balance")); }

void SpringieHandler::SetRandomMap() { m_battle->Say(_T("!map")); }

void SpringieHandler::SetMap(const wxString& map) { m_battle->Say(_T("!map ") + map); }
...
void SpringieHandler::Notify() { m_battle->Say(_T("!notify")); }

void SpringieHandler::Start() { m_battle->Say(_T("!start")); }
...
void SpadsHandler::Balance() { m_battle->Say(_T("!balance")); }

void SpadsHandler::SetRandomMap() {
  m_battle->Say(_T("!map 1")); // not so random
}

void SpadsHandler::SetMap(const wxString& map) { m_battle->Say(_T("!map ") + map); }

void SpadsHandler::ClearStartBoxes() {}

void SpadsHandler::AddStartBox(int /*posx*/, int /*posy*/, int /*w*/, int /*h*/) {}

void SpadsHandler::Notify() { m_battle->Say(_T("!notify")); }

void SpadsHandler::Start() { m_battle->Say(_T("!start")); }
//-------------

AutohostManager::AutohostManager()
  : m_type(AUTOHOSTTYPE_UNKNOWN)
  , m_battle(0) {}
...
SpringieHandler& AutohostManager::GetSpringie() { return m_springie; }

SpadsHandler& AutohostManager::GetSpads() { return m_spads; }
...
AutohostManager::AutohostType AutohostManager::GetAutohostType() { return m_type; }
