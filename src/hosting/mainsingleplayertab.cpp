...
MainSinglePlayerTab::MainSinglePlayerTab(wxWindow* parent) : wxScrolledWindow(parent, -1) {
  m_main_sizer = new wxBoxSizer(wxVERTICAL);
  GetAui().manager->AddPane(this, wxLEFT, _T( "mainsingleplayertab" ));
  m_tabs = new SLNotebook(
      this, _T( "mainsingleplayertab" ), -1, wxDefaultPosition, wxDefaultSize,
...
void MainSinglePlayerTab::ReloadMapOptContrls() {
  try {
    GetMMOptionsTab().OnReloadControls(LSL::OptionsWrapper::MapOption);
  }
  catch (...) {
  }
}
...
void MainSinglePlayerTab::ReloadModOptContrls() {
  try {
    GetMMOptionsTab().OnReloadControls(LSL::OptionsWrapper::ModOption);
  }
  catch (...) {
  }
}
