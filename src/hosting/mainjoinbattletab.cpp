...
MainJoinBattleTab::MainJoinBattleTab(wxWindow* parent)
  ...
  m_tabs = new SLNotebook(
      this, _T( "mainjoinbattletab" ), BATTLE_TABS, wxDefaultPosition, wxDefaultSize,
...
Battle* MainJoinBattleTab::GetCurrentBattle() { return m_battle_tab->GetBattle(); }
...
void MainJoinBattleTab::HostBattle(Battle& battle) { JoinBattle(battle); }
...
void MainJoinBattleTab::PostSwitchBattlePerspective() { ui().mw().LoadPerspectives(); }

void MainJoinBattleTab::PreSwitchBattlePerspective() { ui().mw().SavePerspectives(); }

bool MainJoinBattleTab::UseBattlePerspective() { return GetBattleRoomTab().GetBattle() != 0; }

void MainJoinBattleTab::FocusBattleRoomTab() { m_tabs->SetSelection(0); }
