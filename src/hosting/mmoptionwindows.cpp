...
SingleOptionDialog::SingleOptionDialog(IBattle& battle, const wxString& optiontag)
  ...
  Create((wxWindow*)&ui().mw(), wxID_ANY, _("Change option"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE,
         _T( "OptionDialog" ));
  ...
  switch (type) {
    ...
    case LSL::Enum::opt_float: {
      ...
      m_spinctrl->Create(this, wxID_ANY, _T( "" ), wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, double(opt.min),
                         double(opt.max), double(opt.value), double(opt.stepping), TowxString(opt.key));
    ...
    case LSL::Enum::opt_string: {
      ...
      m_textctrl = new wxTextCtrl(this, wxID_ANY, TowxString(opt.value), wxDefaultPosition, wxDefaultSize, 0,
                                  wxDefaultValidator, TowxString(opt.key));
  ...
  wxStaticLine* m_separator1 =
      new wxStaticLine(this, wxID_ANY, wxDefaultPosition, wxSize(10, -1), wxLI_HORIZONTAL, _T( "ID_STATICLINE1" ));
...
void SingleOptionDialog::OnCancel(wxCommandEvent& /*unused*/) { EndModal(wxID_CANCEL); }
...
void SingleOptionDialog::OnOk(wxCommandEvent& /*unused*/) {
  ...
  if (m_textctrl)
    ...
    value = TowxString(
        m_battle.CustomBattleOptions().GetNameListOptItemKey(key, STD_STRING(m_combobox->GetValue()), optFlag));
  else if (m_spinctrl) {
