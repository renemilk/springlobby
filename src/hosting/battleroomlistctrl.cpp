...
BattleroomListCtrl::BattleroomListCtrl(wxWindow* parent, IBattle* battle, bool readonly, bool showingame)
  : CustomVirtListCtrl<User*, BattleroomListCtrl>(parent, BRLIST_LIST, wxDefaultPosition, wxDefaultSize,
                                                  ...
                                                  _T("BattleroomListCtrl"), 3, &BattleroomListCtrl::CompareOneCrit,
                                                  true /*highlight*/, UserActions::ActHighlight,
                                                  !readonly /*periodic sort*/)
  , m_battle(battle)
  , m_popup(0)
  , m_sel_user(0)
  , m_sides(0)
  ...
  int count = 0;
  AddColumn(count, wxLIST_AUTOSIZE, _T("Status"), _T("Player/Bot"));
  m_status_column_index = count;
  count++;
  if (m_showingame) {
    AddColumn(count, wxLIST_AUTOSIZE, _T("Ingame"), _T("Battleroom status"));
    m_ingame_column_index = count;
    count++;
  }

  AddColumn(count, wxLIST_AUTOSIZE, _T("Faction"), _T("Faction icon"));
  m_faction_column_index = count;
  count++;
  AddColumn(count, wxLIST_AUTOSIZE, _T("Colour"), _T("Teamcolour"));
  m_colour_column_index = count;
  count++;
  AddColumn(count, wxLIST_AUTOSIZE, _T("Country"), _T("Country"));
  m_country_column_index = count;
  count++;
  AddColumn(count, wxLIST_AUTOSIZE, _T("Rank"), _T("Rank"));
  m_rank_column_index = count;
  count++;
  AddColumn(count, wxLIST_AUTOSIZE, _("Nickname"), _T("Ingame name"));
  m_nick_column_index = count;
  count++;
  AddColumn(count, wxLIST_AUTOSIZE, _("Team"), _T("Team number"));
  m_team_column_index = count;
  count++;
  AddColumn(count, wxLIST_AUTOSIZE, _("Ally"), _T("Ally number"));
  m_ally_column_index = count;
  count++;
  AddColumn(count, wxLIST_AUTOSIZE, _("Resource Bonus"), _T("Resource Bonus"));
  ...
  if (!m_ro) {
    ...
    for (unsigned int i = 0; i < SPRING_MAX_TEAMS; i++) {
      wxMenuItem* team =
          new wxMenuItem(m_teams, BRLIST_TEAM + i, wxFormat(_T("%d")) % (i + 1), wxEmptyString, wxITEM_NORMAL);
      m_teams->Append(team);
    ...
    for (unsigned int i = 0; i < SPRING_MAX_ALLIES; i++) {
      wxMenuItem* ally =
          new wxMenuItem(m_allies, BRLIST_ALLY + i, wxFormat(_T("%d")) % (i + 1), wxEmptyString, wxITEM_NORMAL);
...
wxString BattleroomListCtrl::GetItemText(long item, long column) const {
  ...
  if (column == m_nick_column_index) {
    if (is_bot) {
      ...
      if (!user.BattleStatus().aiversion.IsEmpty())
        botname += _T(" ") + user.BattleStatus().aiversion;
      if (!LSL::usync().VersionSupports(LSL::USYNC_GetSkirmishAI)) {
        if (botname.Find(_T('.')) != wxNOT_FOUND)
          botname = botname.BeforeLast(_T('.'));
        if (botname.Find(_T('/')) != wxNOT_FOUND)
          botname = botname.AfterLast(_T('/'));
        if (botname.Find(_T('\\')) != wxNOT_FOUND)
          botname = botname.AfterLast(_T('\\'));
        if (botname.Find(_T("LuaAI:")) != wxNOT_FOUND)
          botname = botname.AfterFirst(_T(':'));
      }
  ...
  if (column == m_team_column_index)
    return is_spec ? _T("") : (wxFormat(_T("%d")) % (user.BattleStatus().team + 1)).c_str();
  if (column == m_ally_column_index)
    return is_spec ? _T("") : (wxFormat(_T("%d")) % (user.BattleStatus().ally + 1)).c_str();
  if (column == m_resourcebonus_column_index)
    return is_spec ? _T("") : (wxFormat(_T("%d%%")) % user.BattleStatus().handicap).c_str();
  if (column == m_country_column_index)
    return _T("");
...
void BattleroomListCtrl::OnListRightClick(wxListEvent& event) {
  ...
  m_popup->EnableItems(!user.BattleStatus().IsBot(), GetSelectedUserNick()); // this updates groups, therefore we need
                                                                             // to update the connection to evt handlers
                                                                             // too
  ...
  Connect(GROUP_ID_NEW, wxEVT_COMMAND_MENU_SELECTED, wxCommandEventHandler(BattleroomListCtrl::OnUserMenuCreateGroup),
          0, this);
  PopupMenu(m_popup);
...
void BattleroomListCtrl::OnTeamSelect(wxCommandEvent& event) {
  ...
  if (m_sel_user)
    ((Battle*)m_battle)->ForceTeam(*m_sel_user, team);
}
...
void BattleroomListCtrl::OnAllySelect(wxCommandEvent& event) {
  ...
  if (m_sel_user)
    ((Battle*)m_battle)->ForceAlly(*m_sel_user, ally);
}
...
void BattleroomListCtrl::OnColourSelect(wxCommandEvent& /*unused*/) {
  ...
  if (!CurrentColour.IsOk())
    return;
  if (m_sel_user)
    ((Battle*)m_battle)->ForceColour(*m_sel_user, CurrentColour);
}
...
void BattleroomListCtrl::OnSideSelect(wxCommandEvent& event) {
  ...
  if (m_sel_user)
    ((Battle*)m_battle)->ForceSide(*m_sel_user, side);
}
...
void BattleroomListCtrl::OnHandicapSelect(wxCommandEvent& /*unused*/) {
  ...
  if (!m_sel_user)
    return;
  long handicap =
      wxGetNumberFromUser(_("Please enter a value between 0 and 100"), _("Set Resource Bonus"), _T(""),
                          m_sel_user->BattleStatus().handicap, 0, 100, (wxWindow*)&ui().mw(), wxDefaultPosition);
  if (handicap != -1) {
...
void BattleroomListCtrl::OnSpecSelect(wxCommandEvent& /*unused*/) {
  wxLogDebugFunc(_T(""));
  if (m_sel_user)
    ((Battle*)m_battle)->ForceSpectator(*m_sel_user, m_spec_item->IsChecked());
}

void BattleroomListCtrl::OnKickPlayer(wxCommandEvent& /*unused*/) {
  wxLogDebugFunc(_T(""));
  if (m_sel_user)
    ((Battle*)m_battle)->KickPlayer(*m_sel_user);
}

void BattleroomListCtrl::OnRingPlayer(wxCommandEvent& /*unused*/) {
  wxLogDebugFunc(_T(""));
  if (m_sel_user)
    ((Battle*)m_battle)->GetServer().Ring(m_sel_user->GetNick());
}
...
int BattleroomListCtrl::CompareOneCrit(DataType u1, DataType u2, int col, int dir) const {
  ...
  if (col == m_nick_column_index)
    return dir * u1->GetNick().CmpNoCase(u2->GetNick());
...
void BattleroomListCtrl::SetTipWindowText(const long item_hit, const wxPoint& position) {
  ...
  if (column > (int)m_colinfovec.size() || column < 0) {
    ...
    {
      m_tiptext = user.BattleStatus().IsBot() ? _T("This bot is from nowhere particular")
                                              : GetFlagNameFromCountryCode(user.GetCountry());
...
void BattleroomListCtrl::OnUserMenuAddToGroup(wxCommandEvent& event) {
  ...
  Disconnect(GROUP_ID_NEW, wxEVT_COMMAND_MENU_SELECTED,
             wxCommandEventHandler(BattleroomListCtrl::OnUserMenuCreateGroup), 0, this);
}
