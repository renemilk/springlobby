...
HostBattleDialog::HostBattleDialog(wxWindow* parent)
  : wxDialog(parent, -1, _("Host new battle"), wxDefaultPosition, wxSize(410, 441),
  ...
  m_pwd_text =
      new wxTextCtrl(m_panel, wxID_ANY, sett().GetLastHostPassword(), wxDefaultPosition, wxDefaultSize, wxTE_PASSWORD);
  ...
  m_port_text = new wxTextCtrl(m_panel, wxID_ANY, wxFormat(_T( "%d" )) % sett().GetLastHostPort(), wxDefaultPosition,
                               wxDefaultSize, 0);
  ...
  m_relayed_host_check =
      new wxCheckBox(m_panel, CHK_USE_RELAY, _("Use relayhost"), wxDefaultPosition, wxDefaultSize, 0);
  ...
  wxMenuItem* automatic_pick = new wxMenuItem(m_relayhost_list, AUTO_PICK_HOST, _("Chose automatically"),
                                              _("Randomly picks an available one"), wxITEM_RADIO);
  m_relayhost_list->Append(automatic_pick);
  automatic_pick->Check(m_last_relayhost.IsEmpty());
  m_relayhost_list->AppendSeparator();
  wxMenuItem* manual_pick_relay =
      new wxMenuItem(m_relayhost_list, MANUAL_PICK_HOST, _("Manually enter the manager name"),
                     _("You'll get prompted for the exact manager name"), wxITEM_RADIO);
  ...
  for (unsigned int i = 0; i < m_relayhost_array_list.GetCount(); i++) {
    wxMenuItem* newitem =
        new wxMenuItem(m_relayhost_list, MANUAL_PICK_HOST + 1 + i, m_relayhost_array_list[i], _T(""), wxITEM_RADIO);
  ...
  m_players_slide =
      new wxSlider(m_panel, wxID_ANY, sett().GetLastHostPlayerNum(), 2, SPRING_MAX_USERS, wxDefaultPosition,
                   wxDefaultSize, wxSL_AUTOTICKS | wxSL_BOTH | wxSL_HORIZONTAL | wxSL_LABELS);
  ...
  m_nat_radios = new wxRadioBox(m_panel, CHOSE_NAT, _("NAT traversal"), wxDefaultPosition, wxDefaultSize,
                                m_nat_radiosNChoices, m_nat_radiosChoices, 1, wxRA_SPECIFY_COLS);
...
void HostBattleDialog::OnCancel(wxCommandEvent& /*unused*/) { EndModal(wxID_CANCEL); }
...
void HostBattleDialog::OnPickRelayHost(wxCommandEvent&) { PopupMenu(m_relayhost_list); }
...
void HostBattleDialog::OnRelayChoice(wxCommandEvent& event) {
  ...
  if (index == AUTO_PICK_HOST)
    ...
    ui().AskText(_("Manually chose a manager"),
                 _("Please type the nick of the manager you want to use ( case sensitive )"), m_last_relayhost);
...
void RunHostBattleDialog(wxWindow* parent) {
  ...
  if (dlg.ShowModal() == wxID_OK) {
      ...
      switch (ui().TestHostPort(bo.port)) {
        ...
        case Server::porttest_pass_WX26:
          ...
          customMessageBoxNoModal(SL_MAIN_ICON,
                                  wxFormat(_("Your using wxWidgets prior to version 2.8,\n "
        ...
        case Server::porttest_unreachable:
          ...
          customMessageBoxNoModal(SL_MAIN_ICON,
                                  wxFormat(_("The server used for testing your port %d "
        ...
        case Server::porttest_socketError:
          ...
          customMessageBoxNoModal(
              SL_MAIN_ICON,
              wxFormat(_("Battle not started because the port you selected (%d) "
                         "is unable to-f /home/kosh/.springlobby/testing.conf recieve incoming packets\n checks your "
                         "router & firewall configuration again or change port "
    ...
    try {
      ...
      customMessageBoxNoModal(SL_MAIN_ICON, _("Battle not started beacuse the game you selected could not be found. "),
                              _("Error starting battle."), wxOK);
      return;
    }
    ...
    try {
      if (LSL::usync().MapExists(mname))
        ...
        customMessageBoxNoModal(SL_MAIN_ICON, _("Couldn't find any maps in your spring installation. This could happen "
                                                "when you set the Spring settings incorrectly."),
