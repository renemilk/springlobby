...
BattleroomMMOptionsTab<BattleType>::BattleroomMMOptionsTab(BattleType* battle, wxWindow* parent, wxWindowID id,
  ...
  m_options_preset_sel = new wxComboBox(this, BOPTS_CHOSEPRES, _T(""), wxDefaultPosition, wxDefaultSize,
...
void BattleroomMMOptionsTab<BattleType>::setupOptionsSizer(wxBoxSizer* parent_sizer,
  ...
  for (const auto& it : m_battle->CustomBattleOptions().m_opts[optFlag].section_map) {
    wxStaticBoxSizer* section_sizer =
        new wxStaticBoxSizer(new wxStaticBox(this, wxID_ANY, TowxString(it.second.name)), wxVERTICAL);
  ...
  if (setupOptionsSectionSizer(dummy, section_sizer, optFlag) == 0) {
    if (num_options == 0) {
      ...
      wxStaticText* none_found =
          new wxStaticText(this, wxID_ANY, _("no options available"), wxDefaultPosition, wxDefaultSize, 0, name);
...
wxButton* BattleroomMMOptionsTab<BattleType>::getButton(const wxWindowID id, const wxString& name) {
  if (!m_battle)
    return 0;
  m_button_map[name] = new wxButton(this, id + BUTTON_ID_OFFSET, _T("?"), wxDefaultPosition, wxDefaultSize,
                                    wxNO_BORDER | wxBU_EXACTFIT, wxDefaultValidator, name);
...
int BattleroomMMOptionsTab<BattleType>::setupOptionsSectionSizer(const LSL::mmOptionSection& section,
  ...
  for (const auto i : optWrap.m_opts[optFlag].bool_map) {
    if (i.second.section == section.key) {
      ...
      ct_sizer->Add(getButton(BOOL_START_ID + ctrl_count, pref + TowxString(current.key)), 0,
                    wxRIGHT | wxALIGN_CENTER_VERTICAL, col_gap);
  ...
  for (const auto it : optWrap.m_opts[optFlag].float_map) {
    ...
    if (it.second.section == section.key) {
      ...
      tempspin->Create(this, FLOAT_START_ID + ctrl_count, _T(""), wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS,
                       double(current.min), double(current.max), double(current.value), double(current.stepping),
                       TowxString(current.key));
  ...
  for (const auto it : optWrap.m_opts[optFlag].list_map) {
    if (it.second.section == section.key) {
      ...
      wxComboBox* tempchoice = new wxComboBox(
          this, LIST_START_ID + ctrl_count, TowxString(current.cbx_choices[index]), wxDefaultPosition, wxDefaultSize,
          LSL::Util::vectorToArrayString(current.cbx_choices), wxCB_READONLY, wxDefaultValidator);
  ...
  for (const auto it : optWrap.m_opts[optFlag].string_map) {
    if (it.second.section == section.key) {
      ...
      wxTextCtrl* temptext =
          new wxTextCtrl(this, STRING_START_ID + ctrl_count, TowxString(current.value), wxDefaultPosition,
...
void BattleroomMMOptionsTab<BattleType>::OnComBoxChange(wxCommandEvent& event) {
  ...
  long gameoption;
  box->GetName().BeforeFirst(sep).ToLong(&gameoption);
  const auto itemKey = m_battle->CustomBattleOptions().GetNameListOptItemKey(
      key, STD_STRING(box->GetValue()), (LSL::OptionsWrapper::GameOption)gameoption);

  if (m_battle->CustomBattleOptions().setSingleOption(key, itemKey, (LSL::OptionsWrapper::GameOption)gameoption)) {
    if (m_battle->IsFounderMe()) {
...
void BattleroomMMOptionsTab<BattleType>::UpdateOptControls(wxString controlName) {
  ...
  if (gameoption == LSL::OptionsWrapper::PrivateOptions) {
    if (optKey == "mapname")
      OnReloadControls(LSL::OptionsWrapper::MapOption);
    if (optKey == "modname")
      OnReloadControls(LSL::OptionsWrapper::ModOption);
    return;
  }
  ...
  if (m_combox_map.find(controlName) != m_combox_map.end()) {
    ...
    cur->SetValue(TowxString(
        (m_battle->CustomBattleOptions().GetNameListOptValue(optKey, (LSL::OptionsWrapper::GameOption)gameoption))));
  }

  if (m_textctrl_map.find(controlName) != m_textctrl_map.end()) {
    const wxString value =
        TowxString(m_battle->CustomBattleOptions().getSingleValue(optKey, (LSL::OptionsWrapper::GameOption)gameoption));
...
void BattleroomMMOptionsTab<BattleType>::OnLoadPreset(wxCommandEvent& /*unused*/) {
  ...
  if (presetname.IsEmpty()) {
    customMessageBoxNoModal(
        SL_MAIN_ICON, _("Cannot load an options set without a name\nPlease select one from the list and try again."),
        _("error"), wxICON_EXCLAMATION | wxOK);
    return;
  }
