...
void ToasterBoxWindow::SetPopupBitmap(wxBitmap& bitmap) { sbm.Create(this, 0, bitmap, wxPoint(0, 0)); }
...
void ToasterBoxWindow::SetPopupSize(int x, int y) { SetSize(bottomRight.x, bottomRight.y, x, y); }

void ToasterBoxWindow::SetPopupPosition(int x, int y) {
  dialogTop = wxPoint(x, y);
  bottomRight = wxPoint(dialogTop.x + GetSize().GetWidth(), dialogTop.y + GetSize().GetHeight());
}
...
void ToasterBoxWindow::PrintInfo() {
  wxLogDebug(_T("brX:%i brY:%i szW:%i szH:%i"), bottomRight.x, bottomRight.y, GetSize().GetWidth(),
             GetSize().GetHeight());
}

void ToasterBoxWindow::OnPaint(wxPaintEvent& /*event*/) { DrawText(); }

/// wxEVT_ERASE_BACKGROUND event handler for ID_WXGRADIENTBUTTON
void ToasterBoxWindow::OnEraseBackground(wxEraseEvent& /*event*/) { DrawText(); }
