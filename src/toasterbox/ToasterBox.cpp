...
void ToasterBox::SetPopupPosition(int x, int y) { popupPosition = wxPoint(x, y); }
...
void ToasterBox::SetPopupPosition(int pos) {
  ...
  // bottom left
  if (pos == 2)
    popupPosition = wxPoint(0, wxGetDisplaySize().GetHeight() - popupSize.GetHeight());
  // bottom right
  if (pos == 3)
    popupPosition = wxPoint(bottomRight.x - popupSize.GetWidth(), bottomRight.y - popupSize.GetHeight());

  bottomRight = wxPoint(popupPosition.x + popupSize.GetWidth(), popupPosition.y + popupSize.GetHeight());
}

void ToasterBox::SetPopupBackgroundColor(int r, int g, int b) { colBg = wxColour(r, g, b); }

void ToasterBox::SetPopupTextColor(int r, int g, int b) { colFg = wxColour(r, g, b); }
...
bool ToasterBox::DoesTextFit() { return false; }

void ToasterBox::MoveAbove(ToasterBoxWindow* tb) {
  // recalc where to place this popup

  if (m_stack_direction == StackUp)
    tb->SetPopupPosition(popupPosition.x, popupPosition.y - (popupSize.GetHeight() * winList->GetCount()));
  else
    tb->SetPopupPosition(popupPosition.x, popupPosition.y + (popupSize.GetHeight() * winList->GetCount()));
}

void ToasterBox::SetStackDirection(StackDirection dir) { m_stack_direction = dir; }
...
void ToasterBox::Notify() {
  ...
  for (int i = node->GetData()->GetPosition().y; i < popupPosition.y; i += 4) {
    ...
    for (unsigned int j = 0; j < winList->GetCount(); j++) {
      ...
      // actually move it
      tmpTb->SetSize(popupPosition.x, ourNewHeight, tmpTb->GetSize().GetWidth(), tmpTb->GetSize().GetHeight());
