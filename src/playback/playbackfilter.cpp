...
PlaybackListFilter<PlaybackTabType>::PlaybackListFilter(wxWindow* parent, wxWindowID id, PlaybackTabType* parentTab,
  ...
  m_filter_player_choice = new wxChoice(this, PLAYBACK_FILTER_PLAYER_CHOICE, wxDefaultPosition, wxSize(-1, -1),
  ...
  m_filter_player_button =
      new wxButton(this, PLAYBACK_FILTER_PLAYER_BUTTON, f_values.player_mode, wxDefaultPosition, wxSize(25, 25), 0);
  ...
  m_filter_map_edit = new wxTextCtrl(this, PLAYBACK_FILTER_MAP_EDIT, f_values.map, wxDefaultPosition, wxSize(-1, -1),
                                     0 | wxSIMPLE_BORDER);
  ...
  m_filter_map_show =
      new wxCheckBox(this, PLAYBACK_FILTER_MAP_SHOW, _("Only maps I have"), wxDefaultPosition, wxSize(-1, -1), 0);
  ...
  m_filter_mod_edit = new wxTextCtrl(this, PLAYBACK_FILTER_MOD_EDIT, f_values.mod, wxDefaultPosition, wxSize(-1, -1),
                                     0 | wxSIMPLE_BORDER);
  ...
  m_filter_mod_show =
      new wxCheckBox(this, PLAYBACK_FILTER_MOD_SHOW, _("Only games I have"), wxDefaultPosition, wxSize(-1, -1), 0);
  ...
  m_filter_filesize_edit = new wxTextCtrl(this, PLAYBACK_FILTER_FILESIZE_EDIT, f_values.filesize, wxDefaultPosition,
                                          wxSize(-1, -1), 0 | wxSIMPLE_BORDER);
  ...
  // button here
  m_filter_filesize_button =
      new wxButton(this, PLAYBACK_FILTER_FILESIZE_BUTTON, f_values.filesize_mode, wxDefaultPosition, wxSize(25, 25), 0);
  ...
  m_filter_duration_text =
      new wxStaticText(this, wxID_ANY, _("Duration (hh:mm:ss):"), wxDefaultPosition, wxSize(-1, -1), 0);
  ...
  m_filter_duration_edit = new wxTextCtrl(this, PLAYBACK_FILTER_DURATION_EDIT, f_values.duration, wxDefaultPosition,
                                          wxSize(-1, -1), 0 | wxSIMPLE_BORDER);
  ...
  m_filter_duration_button =
      new wxButton(this, PLAYBACK_FILTER_DURATION_BUTTON, f_values.duration_mode, wxDefaultPosition, wxSize(25, 25), 0);
...
template <class PlaybackTabType>
typename PlaybackListFilter<PlaybackTabType>::m_button_mode
PlaybackListFilter<PlaybackTabType>::_GetButtonMode(wxString sign) {
  if (sign == _T("<"))
    return m_smaller;
  if (sign == _T(">"))
    return m_bigger;
...
template <class PlaybackTabType>
typename PlaybackListFilter<PlaybackTabType>::m_button_mode
PlaybackListFilter<PlaybackTabType>::_GetNextMode(m_button_mode value) {
  switch (value) {
    case m_equal:
      return m_smaller;
    case m_smaller:
...
template <class PlaybackTabType>
bool PlaybackListFilter<PlaybackTabType>::FilterPlayback(
    const typename PlaybackListFilter<PlaybackTabType>::PlaybackType& playback) {
  ...
  // Map:
  if (!battle.GetHostMapName().Upper().Contains(m_filter_map_edit->GetValue().Upper()) &&
      !m_filter_map_expression->Matches(battle.GetHostMapName()))
    return false;

  // Mod:
  if (!battle.GetHostModName().Upper().Contains(m_filter_mod_edit->GetValue().Upper()) &&
      !battle.GetHostModName().Upper().Contains(m_filter_mod_edit->GetValue().Upper()) &&
      !m_filter_mod_expression->Matches(battle.GetHostModName()))
    return false;

  if ((!m_filter_filesize_edit->GetValue().IsEmpty()) &&
      !_IntCompare(playback.size, 1024 * s2l(m_filter_filesize_edit->GetValue()), m_filter_filesize_mode))
    return false;

  // duration
  if ((!m_filter_duration_edit->GetValue().IsEmpty()) &&
      !_IntCompare(playback.duration, m_duration_value, m_filter_duration_mode))
    return false;
...
void PlaybackListFilter<PlaybackTabType>::SetDurationValue() {
  ...
  switch (sep_count) {
    ...
    case 1:
      ...
      break;
    case 2:
      m_duration_value = s2l(dur.AfterLast(*mysep)) + (s2l(dur.AfterFirst(*mysep).BeforeFirst(*mysep)) * 60) +
