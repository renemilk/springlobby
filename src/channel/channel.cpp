...
void Channel::SetName(const wxString& name) { m_name = name; }

wxString Channel::GetName() { return m_name; }

User& Channel::GetMe() { return m_serv.GetMe(); }

void Channel::Said(User& who, const wxString& message) { ui().OnChannelSaid(*this, who, message); }
...
void Channel::DidAction(User& who, const wxString& action) { ui().OnChannelDidAction(*this, who, action); }
...
void Channel::Leave() { m_serv.PartChannel(m_name); }

void Channel::Rejoin() { m_serv.JoinChannel(m_name, m_password); }
...
wxString Channel::GetTopicSetBy() { return m_topic_nick; }

wxString Channel::GetTopic() { return m_topic; }
...
void Channel::RemoveUser(const wxString& nick) { UserList::RemoveUser(nick); }
  ...
  if (param == _T("/me")) {
    ...
    if (m_do_ban_regex) {
      ...
      if (!m_ban_regex.IsValid())
        ui().OnChannelMessage(m_name, _T("Invalid regular expression"));
    }
    return true;
...
wxString Channel::GetPassword() { return m_password; }

void Channel::SetPassword(const wxString& pw) { m_password = pw; }
