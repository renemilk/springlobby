...
#ifndef __WXDEBUG__
#define wxLogDebugFunc(params)                                                                                         \
  wxLogVerbose(_T("%s"), wxString(wxString(__FUNCTION__, wxConvUTF8) + _T(" ( ") + wxString(params) + _T(" )")).c_str())
#else
...
#define ASSERT_EXCEPTION(cond, msg)                                                                                    \
  if (!(cond)) {                                                                                                       \
    wxLogMessage(_T("runtime assertion ( %s:%d ): %s"), TowxString(__FILE__).c_str(), __LINE__,                        \
                 wxString(msg).c_str());                                                                               \
