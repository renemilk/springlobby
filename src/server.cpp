...
User& Server::GetUser(const wxString& nickname) const { return m_users.GetUser(nickname); }

bool Server::UserExists(const wxString& nickname) const { return m_users.UserExists(nickname); }

Channel& Server::GetChannel(const wxString& name) { return m_channels.GetChannel(name); }

int Server::GetNumChannels() const { return m_channels.GetNumChannels(); }

Channel& Server::GetChannel(const int& index) { return m_channels.GetChannel(index); }

bool Server::ChannelExists(const wxString& name) const { return m_channels.ChannelExists(name); }

Battle& Server::GetBattle(const int& battleid) { return battles_iter->GetBattle(battleid); }

bool Server::BattleExists(const int& battleid) const { return battles_iter->BattleExists(battleid); }
...
wxArrayString Server::GetRelayHostList() {
  if (UserExists(_T("RelayHostManagerList")))
    SayPrivate(_T("RelayHostManagerList"), _T("!listmanagers"));
  wxArrayString ret;
...
ServerSelector::ServerSelector() : m_serv(0) {}

bool ServerSelector::GetServerStatus() const { return (bool)(m_serv); }
