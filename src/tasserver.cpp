...
myteamcolor:  Should be 32-bit signed integer in decimal form (e.g. 255 and not FF) where each color channel should
occupy 1 byte (e.g. in hexdecimal: $00BBGGRR, B = blue, G = green, R = red). Example: 255 stands for $000000FF.
...
TASServer::TASServer(int serverEventsMode)
  ...
  , m_buffer(_T(""))
  , m_last_udp_ping(0)
  , m_last_ping(time(0) - PING_TIME + PING_DELAY)
...
bool TASServer::ExecuteSayCommand(const wxString& cmd) {
  ...
  if (subcmd == _T("/ingame")) {
    ...
    if (arrayparams.GetCount() < 4)
      return false;
    if (arrayparams.GetCount() > 5)
      return false;
    ...
    sett().SetServerAccountNick(sett().GetDefaultServer(), arrayparams[1]); // this code assumes that default server
                                                                            // hasn't changed since login ( like it
                                                                            // should atm )
    return true;
...
bool TASServer::IsConnected() { return (m_sock->State() == SS_Open) && (m_connected); }

bool TASServer::Register(const wxString& addr, const int port, const wxString& nick, const wxString& password,
                         wxString& reason) {
  wxLogDebugFunc(_T(""));
  FakeNetClass temp;
  ...
  if (data.Find(_T("\r")) != wxNOT_FOUND)
    data = data.BeforeLast(_T('\r'));
...
User& TASServer::GetMe() { return GetUser(m_user); }
const User& TASServer::GetMe() const { return GetUser(m_user); }
...
void TASServer::RequestChannels() { SendCmd(_T("CHANNELS")); }

void TASServer::AcceptAgreement() { SendCmd(_T("CONFIRMAGREEMENT")); }
  ...
  } else // We are connected already.
  {
    ...
    if (m_do_finalize_join_battle && (m_last_udp_ping + udp_reply_timeout < now)) {
      customMessageBoxNoModal(
          SL_MAIN_ICON,
          _("Failed to punch through NAT, playing this battle might not work for you or for other players."),
      ...
      if (m_battle_id != -1) {
        ...
        if (battle) {
          if ((battle->GetNatType() == NAT_Hole_punching || (battle->GetNatType() == NAT_Fixed_source_ports)) &&
            ...
            // if(battle->GetNatType()!=NAT_Hole_punching)wxLogMessage( _T("pinging: current battle not using
            // NAT_Hole_punching") );
...
void TASServer::ExecuteCommand(const wxString& in) {
  ...
  if (m_debug_dont_catch) {
    ...
    catch (...) // catch everything so the app doesn't crash, may makes odd beahviours but it's better than crashing
                // randomly for normal users
    {
    }
  }
}
...
void TASServer::ExecuteCommand(const wxString& cmd, const wxString& inparams, int replyid) {
  ...
  if (cmd == _T("TASSERVER")) {
    ...
    m_se->OnBattleOpenedEx(id, (BattleType)type, IntToNatType(nat), nick, host, port, maxplayers, haspass, rank, hash,
                           wxEmptyString, wxEmptyString, map, title, mod);
    if (nick == m_relay_host_bot) {
    ...
    mod = GetSentenceParam(params);
    m_se->OnBattleOpenedEx(id, (BattleType)type, IntToNatType(nat), nick, host, port, maxplayers, haspass, rank, hash,
                           engineName, engineVersion, map, title, mod);
    if (nick == m_relay_host_bot) {
      GetBattle(id).SetProxy(m_relay_host_bot);
      JoinBattle(id, sett().GetLastHostPassword()); // autojoin relayed host battles
    }
    ...
    if (sett().GetReportStats()) {
      ...
      if (UserExists(_T("SL_bot")))
        SayPrivate(_T("SL_bot"), reportstring);
    }
    if (UserExists(_T("RelayHostManagerList")))
      SayPrivate(_T("RelayHostManagerList"), _T("!lm"));
    ...
    if ((nick == _T("RelayHostManagerList")) && (params == _T("!lm")))
      return; // drop the message
    if (nick == _T("SL_bot")) {
    ...
    try {
      if (GetBattle(id).IsProxy())
        RelayCmd(_T("SUPPORTSCRIPTPASSWORD")); // send flag to relayhost marking we support script passwords
    }
    catch (...) {
    }
    ...
    //"DISABLEUNITS" params: "arm_advanced_radar_tower arm_advanced_sonar_station arm_advanced_torpedo_launcher
    // arm_dragons_teeth arm_energy_storage arm_eraser arm_fark arm_fart_mine arm_fibber arm_geothermal_powerplant
    // arm_guardian"
    while ((nick = GetWordParam(params)) != _T("")) {
    ...
    if (ai.IsEmpty()) {
      wxLogWarning(
          wxString::Format(_T("Recieved illegal ADDBOT (empty dll field) from %s for battle %d"), nick.c_str(), id));
    ...
    m_se->OnBattleInfoUpdated(m_battle_id);
    // !! Command: "SETSCRIPTTAGS" params: "game/startpostype=0	game/maxunits=1000	game/limitdgun=0
    // game/startmetal=1000
    // game/gamemode=0	game/ghostedbuildings=-1	game/startenergy=1000	game/diminishingmms=0"
...
void TASServer::ModeratorMute(const wxString& channel, const wxString& nick, int duration, bool byip) {
  SendCmd(_T("MUTE"),
          channel + _T(" ") + nick + _T(" ") + wxString::Format(_T("%d"), duration) + (byip ? _T(" ip") : _T("")));
}
...
void TASServer::ModeratorGetIP(const wxString& nick) { SendCmd(_T("GETIP"), nick); }

void TASServer::ModeratorGetLastLogin(const wxString& nick) { SendCmd(_T("GETLASTLOGINTIME"), nick); }

void TASServer::ModeratorGetLastIP(const wxString& nick) { SendCmd(_T("GETLASTIP"), nick); }

void TASServer::ModeratorFindByIP(const wxString& ipadress) { SendCmd(_T("FINDIP"), ipadress); }
...
void TASServer::HostBattle(BattleOptions bo, const wxString& password) {
  ...
  // FIXME: this is a dirty hack for https://github.com/spring/LobbyProtocol/issues/15 which breaks compatibility to
  // other lobbies
...
void TASServer::FinalizeJoinBattle() {
  if (m_do_finalize_join_battle) {
    ...
    SendCmd(_T("JOINBATTLE"), wxString::Format(_T("%d %s %s"), m_finalize_join_battle_id,
                                               m_finalize_join_battle_pw.c_str(), randomScriptPassword.c_str()));
    m_do_finalize_join_battle = false;
  }
}
...
void TASServer::SendHostInfo(HostInfo update) {
  ...
  if ((update & (IBattle::HI_Map | IBattle::HI_Locked | IBattle::HI_Spectators)) > 0) {
    ...
    if (!battle.IsProxy())
      ...
      RelayCmd(_T("UPDATEBATTLEINFO"), cmd);
  }
  ...
  if ((update & IBattle::HI_StartRects) > 0) // Startrects should be updated.
  {
    ...
    for (unsigned int i = 0; i <= numrects; i++) // Loop through all, and remove updated or deleted.
    {
      ...
      if (sr.todelete) {
        ...
        if (!battle.IsProxy())
          SendCmd(_T("ADDSTARTRECT"),
                  wxString::Format(_T("%d %d %d %d %d"), sr.ally, sr.left, sr.top, sr.right, sr.bottom));
        else
          RelayCmd(_T("ADDSTARTRECT"),
                   wxString::Format(_T("%d %d %d %d %d"), sr.ally, sr.left, sr.top, sr.right, sr.bottom));
  ...
  if ((update & IBattle::HI_Restrictions) > 0) {
    ...
    if (!battle.IsProxy())
      ...
      RelayCmd(_T("ENABLEALLUNITS"));
    if (!units.empty()) {
      wxString msg;
      wxString scriptmsg;
      for (std::map<wxString, int>::const_iterator itor = units.begin(); itor != units.end(); ++itor) {
        msg << itor->first + _T(" ");
        scriptmsg << _T("game/restrict/") + itor->first + _T("=") + TowxString(itor->second) +
                         _T('\t'); // this is a serious protocol abuse, but on the other hand, the protocol fucking suck
                                   // and it's unmaintained so it will do for now
      }
      if (!battle.IsProxy()) {
...
void TASServer::SendRaw(const wxString& raw) { SendCmd(raw); }

void TASServer::RequestInGameTime(const wxString& nick) { SendCmd(_T("GETINGAMETIME"), nick); }
...
void TASServer::ForceTeam(int battleid, User& user, int team) {
  ...
  if (!GetBattle(battleid).IsFounderMe()) {
    DoActionBattle(battleid, _T("suggests that ") + user.GetNick() + _T(" changes to team #") +
                                 wxString::Format(_T("%d"), team + 1) + _T("."));
    return;
  }

  // FORCETEAMNO username teamno
  if (!GetBattle(battleid).IsProxy())
    SendCmd(_T("FORCETEAMNO"), user.GetNick() + wxString::Format(_T(" %d"), team));
  else
    RelayCmd(_T("FORCETEAMNO"), user.GetNick() + wxString::Format(_T(" %d"), team));
}
...
void TASServer::ForceAlly(int battleid, User& user, int ally) {
  ...
  if (!GetBattle(battleid).IsFounderMe()) {
    DoActionBattle(battleid, _T("suggests that ") + user.GetNick() + _T(" changes to ally #") +
                                 wxString::Format(_T("%d"), ally + 1) + _T("."));
    return;
  }

  // FORCEALLYNO username teamno
  if (!GetBattle(battleid).IsProxy())
    SendCmd(_T("FORCEALLYNO"), user.GetNick() + wxString::Format(_T(" %d"), ally));
  else
    RelayCmd(_T("FORCEALLYNO"), user.GetNick() + wxString::Format(_T(" %d"), ally));
}
...
void TASServer::ForceColour(int battleid, User& user, const wxColour& col) {
  ...
  if (!GetBattle(battleid).IsProxy())
    SendCmd(_T("FORCETEAMCOLOR"), user.GetNick() + _T(" ") + wxString::Format(_T("%d"), tascl.data));
  else
    RelayCmd(_T("FORCETEAMCOLOR"), user.GetNick() + _T(" ") + wxString::Format(_T("%d"), tascl.data));
}
...
void TASServer::ForceSpectator(int battleid, User& user, bool spectator) {
  ...
  if (!GetBattle(battleid).IsProxy())
    SendCmd(_T("FORCESPECTATORMODE"), user.GetNick());
  else
    RelayCmd(_T("FORCESPECTATORMODE"), user.GetNick());
}
...
void TASServer::BattleKickPlayer(int battleid, User& user) {
  ...
  if (!GetBattle(battleid).IsProxy()) {
    user.BattleStatus().scriptPassword = wxString::Format(
        _T("%04x%04x"), rand() & 0xFFFF, rand() & 0xFFFF); // reset his password to something random, so he can't rejoin
    SetRelayIngamePassword(user);
    SendCmd(_T("KICKFROMBATTLE"), user.GetNick());
  } else
    RelayCmd(_T("KICKFROMBATTLE"), user.GetNick());
}
...
void TASServer::SetHandicap(int battleid, User& user, int handicap) {
  ...
  if (!GetBattle(battleid).IsFounderMe()) {
    DoActionBattle(battleid, _T("thinks ") + user.GetNick() + _T(" should get a ") +
                                 wxString::Format(_T("%d"), handicap) + _T("% resource bonus"));
    return;
  }

  // HANDICAP username value
  if (!GetBattle(battleid).IsProxy())
    SendCmd(_T("HANDICAP"), user.GetNick() + wxString::Format(_T(" %d"), handicap));
  else
    RelayCmd(_T("HANDICAP"), user.GetNick() + wxString::Format(_T(" %d"), handicap));
}
...
void TASServer::RemoveBot(int battleid, User& bot) {
  ...
  // REMOVEBOT name
  if (!GetBattle(battleid).IsProxy())
    SendCmd(_T("REMOVEBOT"), bot.GetNick());
  else
    RelayCmd(_T("REMOVEBOT"), bot.GetNick());
}
...
void TASServer::UpdateBot(int battleid, User& bot, UserBattleStatus& status) {
  ...
  if (!GetBattle(battleid).IsProxy())
    SendCmd(_T("UPDATEBOT"), bot.GetNick() + wxString::Format(_T(" %d %d"), tasbs.data, tascl.data));
  else
    RelayCmd(_T("UPDATEBOT"), bot.GetNick() + wxString::Format(_T(" %d %d"), tasbs.data, tascl.data));
}

void TASServer::SendScriptToProxy(const wxString& script) {
  wxArrayString strings = wxStringTokenize(script, _T("\n"));
  int relaylenghtprefix =
      10 + 1 + m_relay_host_bot.Len() + 2; // SAYPRIVATE + space + botname + space + exclamation mark lenght
  ...
  DoActionBattle(
      m_battle_id,
      wxString::Format(_T("is preparing to start the game, game will start in approximately %d seconds"), time));
...
unsigned int
TASServer::UdpPing(unsigned int src_port, const wxString& target, unsigned int target_port,
                   const wxString& message) // full parameters version, used to ping all clients when hosting.
{
  int result = 0;
  wxLogMessage(_T("UdpPing src_port=%d , target='%s' , target_port=%d , message='%s'"), src_port, target.c_str(),
               target_port, message.c_str());
...
void TASServer::UdpPingAllClients() // used when hosting with nat holepunching. has some rudimentary support for fixed
                                    // source ports.
{
  Battle* battle = GetCurrentBattle();
  if (!battle)
  ...
  for (unsigned int i = 0; i < m_relay_host_manager_list.GetCount(); i++) {
    try {
      ...
      if (manager.Status().in_game)
        continue; // skip the manager is not connected or reports it's ingame ( no slots available ), or it's away (
