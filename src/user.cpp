...
void User::Say(const wxString& message) const { GetServer().SayPrivate(m_nick, message); }

void User::DoAction(const wxString& message) const { GetServer().DoActionPrivate(m_nick, message); }

Battle* User::GetBattle() const { return m_battle; }
...
void CommonUser::UpdateBattleStatus(const UserBattleStatus& status) {
  ...
  if (!status.airawname.IsEmpty())
    m_bstatus.airawname = status.airawname;
  if (!status.aiversion.IsEmpty())
...
void User::SendMyUserStatus() const { GetServer().SendMyUserStatus(); }
...
wxString User::GetRankName(UserStatus::RankContainer rank) {
  ...
  switch (rank) {
    ...
    case UserStatus::RANK_5:
      return _("Experienced");
    case UserStatus::RANK_6:
      return _("Highly experienced");
    case UserStatus::RANK_7:
      return _("Veteran");
    case UserStatus::RANK_8:
      return _("Badly needs to get laid");
    default:
      return _("Unknown");
  }
}
...
void CommonUser::SetStatus(const UserStatus& status) { m_status = status; }
