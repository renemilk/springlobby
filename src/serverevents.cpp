...
void ServerEvents::OnConnected(const wxString& server_name, const wxString& server_ver, bool supported,
                               const wxString& server_spring_ver, bool /*unused*/) {
...
void ServerEvents::OnPong(wxLongLong ping_time) {
  // wxLongLong is non-POD and cannot be passed to wxFormat as such. use c-string rep instead. converting to long might
  // loose precision
        ...
        if (battle.GetFounder().GetNick() == user.GetNick()) {
          if (status.in_game != battle.GetInGame()) {
            ...
            if (status.in_game)
              ...
              BattleEvents::GetBattleEventSender(BattleEvents::BattleInfoUpdate)
                  .SendEvent(std::make_pair(user.GetBattle(), wxString()));
          }
        }
      }
      catch (...) {
...
void ServerEvents::OnBattleOpenedEx(int id, BattleType type, NatType nat, const wxString& nick, const wxString& host,
                                    int port, int maxplayers, bool haspass, int rank, const wxString& maphash,
                                    const wxString& engineName, const wxString& engineVersion, const wxString& map,
...
void ServerEvents::OnBattleInfoUpdated(int battleid, int spectators, bool locked, const wxString& maphash,
                                       const wxString& map) {
  wxLogDebugFunc(_T(""));
  try {
    Battle& battle = m_serv.GetBattle(battleid);
  ...
  try {
    ...
    if (key.Left(5) == _T("game/")) {
      ...
      if (key.Left(11) == _T( "mapoptions/" )) {
        key = key.AfterFirst('/');
        battle.CustomBattleOptions().setSingleOption(STD_STRING(key), STD_STRING(value),
                                                     LSL::OptionsWrapper::MapOption);
        battle.Update(wxString::Format(_T("%d_%s"), LSL::OptionsWrapper::MapOption, key.c_str()));
      } else if (key.Left(11) == _T( "modoptions/" )) {
        key = key.AfterFirst('/');
        battle.CustomBattleOptions().setSingleOption(STD_STRING(key), STD_STRING(value),
                                                     LSL::OptionsWrapper::ModOption);
...
void ServerEvents::OnChannelTopic(const wxString& channel, const wxString& who, const wxString& message,
                                  int /*unused*/) {
  wxLogDebugFunc(_T(""));
  try {
...
void ServerEvents::OnBattleAction(int /*battleid*/, const wxString& nick, const wxString& msg) {
  try {
    UiEvents::GetUiEventSender(UiEvents::OnBattleActionEvent).SendEvent(UiEvents::OnBattleActionData(nick, msg));
  }
  catch (assert_exception) {
  }
}
...
void ServerEvents::OnAcceptAgreement(const wxString& agreement) { ui().OnAcceptAgreement(agreement); }

void ServerEvents::OnRing(const wxString& from) { ui().OnRing(from); }

void ServerEvents::OnServerBroadcast(const wxString& message) { ui().OnServerBroadcast(m_serv, message); }

void ServerEvents::OnServerMessage(const wxString& message) { ui().OnServerMessage(m_serv, message); }

void ServerEvents::OnServerMessageBox(const wxString& message) { ui().ShowMessage(_("Server Message"), message); }
...
void ServerEvents::OnHostExternalUdpPort(const unsigned int udpport) {
  if (!m_serv.GetCurrentBattle())
    return;
  if (m_serv.GetCurrentBattle()->GetNatType() == NAT_Hole_punching ||
      m_serv.GetCurrentBattle()->GetNatType() == NAT_Fixed_source_ports)
  ...
  try {
    ...
    if (sett().GetShowIPAddresses()) {
      UiEvents::GetUiEventSender(UiEvents::OnBattleActionEvent)
          .SendEvent(UiEvents::OnBattleActionData(username, wxString::Format(_(" has ip=%s"), ip.c_str())));
    }

    if (m_serv.GetCurrentBattle()->GetNatType() != NAT_None && (udpport == 0)) {
      // todo: better warning message
      // something.OutputLine( _T(" ** ") + who.GetNick() + _(" does not support nat traversal! ") + GetChatTypeStr() +
      // _T("."), sett().GetChatColorJoinPart(), sett().GetChatFont() );
      UiEvents::GetUiEventSender(UiEvents::OnBattleActionEvent)
          .SendEvent(UiEvents::OnBattleActionData(username, _(" does not really support nat traversal")));
    }
...
void ServerEvents::OnRedirect(const wxString& address, unsigned int port, const wxString& CurrentNick,
...
void ServerEvents::OnMutelistEnd(const wxString& channel) { mutelistWindow(_("End mutelist for ") + channel); }
  ...
  if (code != 0) {
    ...
    if (m_autolaunch) {
      if (!wxExecute(_T("\"") + m_savepath + _T("\""), wxEXEC_ASYNC)) {
        customMessageBoxNoModal(SL_MAIN_ICON, _("Couldn't launch installer. File location is: ") + m_savepath,
                                _("Couldn't launch installer."));
      }
    }
    if (m_autoclose) {
      ui().mw().Close();
...
void ServerEvents::OnForceJoinBattle(int battleid, const wxString& scriptPW) {
  ...
  UiEvents::GetStatusEventSender(UiEvents::addStatusMessage)
      .SendEvent(UiEvents::StatusData(_("Automatically moved to new battle"), 1));
}
