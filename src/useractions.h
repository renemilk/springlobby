...
const wxString m_actionNames[] = {_("none"),      _("highlight"), _("notify login/out"),     _("ignore chat"),
                                  _("ignore pm"), _("autokick"),  _("notify hosted battle"), _("notify status change")};

//! Provide the names to be used by config file.
const wxString m_configActionNames[] = {_T("none"),      _T("highlight"), _T("notify_login"),  _T("ignore_chat"),
                                        _T("ignore_pm"), _T("autokick"),  _T("notify_hosted"), _T("notify_status")};

//!same for tooltips
const wxString m_actionTooltips[] = {
    _("no action at all"),
    _("highlight user in nick list and battles he participates in"),
    _("popup a message box when user logs in/out from  the server"),
    _("you won't see message by these users in normal channels"),
    _("ignore private messages of these users, no pm window will open if any of these try to contact you privately"),
    _("automatically kick users from battles hosted by yourself"),
    _("popup a message box when user hosts a new battle"),
...
/** one single static instance is exposed as a global \n
    ...
    currently Gui updates are handled old fashoined way by hangling around classes, this should be improved to dynamic
events
