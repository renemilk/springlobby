...
struct equal_types {
  enum {
    value =
        (sizeof(equal_types_helper(equal_types_ptr<T>::make(), equal_types_ptr<U>::make())) == sizeof(equal_types_yes))
  };
};
...
template <bool test>
struct selector_switch {
  typedef select_then result;
};

template <>
struct selector_switch<false> {
...
public:
  ...
  operator inv_pair<second_type, first_type>&() { return *reinterpret_cast<inv_pair<second_type, first_type>*>(this); }
  ...
  public:
    ...
    from_binding& operator=(const typename bimap_type::to_type& t) {
      ...
      if (tit != bm.tset.end()) {/* v.second shouldn't be already in */
        ...
        if (fit != bm.fset.end() && bimap_type::element_by_from(*fit) == bimap_type::element_by_to(*tit)) {
          return *this;
        } else
      ...
      try {
        ...
        if (fit == bm.fset.end()) {
          ...
          const_cast<typename bimap_type::from_type*&>(*fit) = &const_cast<typename bimap_type::from_type&>(pne->first);
          return *this;
        }
      }
      catch (...) {
  ...
  public:
    ...
    to_binding& operator=(const typename bimap_type::from_type& f) {
      ...
      if (fit != bm.fset.end()) {/* v.second shouldn't be already in */
        /* small chance the pair (f,t) is already inserted */
        if (tit != bm.tset.end() && bimap_type::element_by_to(*tit) == bimap_type::element_by_from(*fit)) {
          return *this;
        } else
          throw duplicate_value();
      }
      ...
      try {
        ...
        if (tit == bm.tset.end()) {
          ...
          bm.delete_element(pe);
          const_cast<typename bimap_type::to_type*&>(*tit) = &const_cast<typename bimap_type::to_type&>(pne->second);
          return *this;
        }
      }
      catch (...) {
...
private:
  ...
  typedef std::set<const from_type*, p_compare<from_type, from_compare>,
                   BIMAP_REBIND_9B698EF9_C6E9_4BC4_A7D2_5B4D71155761(allocator_type_, const from_type*)> from_set;
  typedef std::set<const to_type*, p_compare<to_type, to_compare>,
                   BIMAP_REBIND_9B698EF9_C6E9_4BC4_A7D2_5B4D71155761(allocator_type_, const to_type*)> to_set;
  ...
  static element* element_by_from(const from_type* pf) {
    return reinterpret_cast<element*>(reinterpret_cast<char*>(const_cast<from_type*>(pf)) -
                                      BIMAP_OFFSETOF_9B698EF9_C6E9_4BC4_A7D2_5B4D71155761(element, first));
  }

  static element* element_by_to(const to_type* pt) {
    return reinterpret_cast<element*>(reinterpret_cast<char*>(const_cast<to_type*>(pt)) -
...
public:
  ...
  public:
    ...
    bool operator==(const from_impl& r) const { return size() == r.size() && std::equal(begin(), end(), r.begin()); }

    bool operator!=(const from_impl& r) const { return !(*this == r); }

    bool operator<(const from_impl& r) const {
      return std::lexicographical_compare(begin(), end(), r.begin(), r.end());
    }

    bool operator>(const from_impl& r) const { return r < *this; }
...
#if defined(_MSC_VER) && _MSC_VER == 1200 /* MSVC++ 6.0 */
    typedef std::reverse_bidirectional_iterator<const_iterator, const value_type, const_reference, const value_type*,
    ...
    operator[](const from_type_& f) {
      return from_binding<prebimap<from_type_, to_type_, from_compare, to_compare, allocator_type_>>(owner(), f);
    }

    const_from_binding<prebimap<from_type_, to_type_, from_compare, to_compare, allocator_type_>>
    operator[](const from_type_& f) const {
      return const_from_binding<prebimap<from_type_, to_type_, from_compare, to_compare, allocator_type_>>(owner(), f);
    }
    ...
    key_compare key_comp() const { return owner().fset.key_comp().get_compare(); }

    value_compare value_comp() const {
      return value_compare(owner().fset.key_comp().get_compare(), owner().tset.key_comp().get_compare());
    }

    iterator find(const key_type& key) { return iterator(owner().fset.find(&key)); }

    const_iterator find(const key_type& key) const { return const_iterator(owner().fset.find(&key)); }

    size_type count(const key_type& key) const { return owner().fset.count(&key); }

    iterator lower_bound(const key_type& key) { return iterator(owner().fset.lower_bound(&key)); }

    const_iterator lower_bound(const key_type& key) const { return const_iterator(owner().fset.lower_bound(&key)); }

    iterator upper_bound(const key_type& key) { return iterator(owner().fset.upper_bound(&key)); }

    const_iterator upper_bound(const key_type& key) const { return const_iterator(owner().fset.upper_bound(&key)); }
  ...
  protected:
    ...
    prebimap& owner() {
      return *reinterpret_cast<prebimap*>(reinterpret_cast<char*>(this) -
                                          BIMAP_OFFSETOF_9B698EF9_C6E9_4BC4_A7D2_5B4D71155761(prebimap, from));
    }
    ;
    const prebimap& owner() const {
      return *reinterpret_cast<const prebimap*>(reinterpret_cast<const char*>(this) -
                                                BIMAP_OFFSETOF_9B698EF9_C6E9_4BC4_A7D2_5B4D71155761(prebimap, from));
    }
    ;
  ...
  public:
    friend void swap(from& x, from& y) { x.swap(y); }
  ...
  public:
    ...
    bool operator==(const to_impl& r) const { return size() == r.size() && std::equal(begin(), end(), r.begin()); }

    bool operator!=(const to_impl& r) const { return !(*this == r); }

    bool operator<(const to_impl& r) const { return std::lexicographical_compare(begin(), end(), r.begin(), r.end()); }

    bool operator>(const to_impl& r) const { return r < *this; }
    ...
    operator[](const to_type_& t) {
      return to_binding<prebimap<from_type_, to_type_, from_compare, to_compare, allocator_type_>>(owner(), t);
    }

    const_to_binding<prebimap<from_type_, to_type_, from_compare, to_compare, allocator_type_>>
    operator[](const to_type_& t) const {
      return const_to_binding<prebimap<from_type_, to_type_, from_compare, to_compare, allocator_type_>>(owner(), t);
    }
    ...
    key_compare key_comp() const { return owner().tset.key_comp().get_compare(); }

    value_compare value_comp() const {
      return value_compare(owner().tset.key_comp().get_compare(), owner().fset.key_comp().get_compare());
    }

    iterator find(const key_type& key) { return iterator(owner().tset.find(&key)); }

    const_iterator find(const key_type& key) const { return const_iterator(owner().tset.find(&key)); }

    size_type count(const key_type& key) const { return owner().tset.count(&key); }

    iterator lower_bound(const key_type& key) { return iterator(owner().tset.lower_bound(&key)); }

    const_iterator lower_bound(const key_type& key) const { return const_iterator(owner().tset.lower_bound(&key)); }

    iterator upper_bound(const key_type& key) { return iterator(owner().tset.upper_bound(&key)); }

    const_iterator upper_bound(const key_type& key) const { return const_iterator(owner().tset.upper_bound(&key)); }
  ...
  public:
    friend void swap(to& x, to& y) { x.swap(y); }
  } to; /* to memberspace */
  ...
  prebimap(const prebimap& r)
    ...
    try {
      ...
      for (typename from_impl::const_iterator it = r.from.begin(); it != r.from.end(); ++it) {
        std::pair<typename from_impl::iterator, typename to_impl::iterator> p = insert(fhint, thint, *it);
        fhint = p.first;
        thint = p.second;
      }
#endif
...
template <typename from_type, typename to_type, typename from_compare, typename to_compare, typename allocator_type>
...
#endif
    {
  typedef prebimap<from_type_, to_type_, from_compare, to_compare, allocator_type_> super;
...
public:
  ...
  std::pair<iterator, iterator> equal_range(const key_type& key) { return this->from.equal_range(key); }
  std::pair<const_iterator, const_iterator> equal_range(const key_type& key) const {
  ...
  std::pair<to_iterator, bool> insert(const to_value_type& x) { return this->to.insert(x); }
  to_iterator insert(to_iterator it, const to_value_type& x) { return this->to.insert(it, x); }
  void insert(const to_value_type* first, const to_value_type* last) { this->to.insert(first, last); }
...
public:
  ...
  friend void swap(bimap_different_types& x, bimap_different_types& y) { x.swap(y); }
  key_compare key_comp() const { return this->from.key_comp(); }
  value_compare value_comp() const { return this->from.value_comp(); }
  iterator find(const key_type& key) { return this->from.find(key); }
  const_iterator find(const key_type& key) const { return this->from.find(key); }
  ...
  std::pair<to_iterator, to_iterator> equal_range(const to_key_type& key) { return this->to.equal_range(key); }
 ...
 * simulate PTS.
 */
template <typename from_type_, typename to_type_, typename from_compare = std::less<from_type_>,
...
class bimap : public bimap_detail::select<
                  ...
                  bimap_equal_types<from_type_, to_type_, from_compare, to_compare, allocator_type>,
                  bimap_different_types<from_type_, to_type_, from_compare, to_compare, allocator_type>>::result {
protected:
  typedef typename bimap_detail::select<
      bimap_detail::equal_types<from_type_, to_type_>::value,
      bimap_equal_types<from_type_, to_type_, from_compare, to_compare, allocator_type>,
      bimap_different_types<from_type_, to_type_, from_compare, to_compare, allocator_type>>::result super;

public:
  explicit bimap(const from_compare& from_comp = from_compare(), const to_compare& to_comp = to_compare(),
...
/* inverse copy ctor (from a bimap<to_type,from_type>) */
  ...
  typedef bimap<to_type_, from_type_, to_compare, from_compare,
                std::allocator<direct_pair<const to_type_, const from_type_>>> inv_bimap;
  ...
  explicit bimap(const inv_bimap& r)
    ...
    typedef bimap<
        ...
        typename allocator_type::template rebind<direct_pair<const to_type_, const from_type_>>::other> inv_bimap;
    ...
    /* body of bimap(const inv_bimap& r) follows */
  {
    try {
      ...
      for (typename inv_bimap::to::const_iterator it = r.to.begin(); it != r.to.end(); ++it) {
        std::pair<typename super::from::iterator, typename super::to::iterator> p = this->insert(fhint, thint, *it);
        fhint = p.first;
        thint = p.second;
      }
#endif
  ...
  template <typename it_type>
  bimap(it_type first, it_type last, const from_compare& from_comp = from_compare(),
        const to_compare& to_comp = to_compare(), const allocator_type& al = allocator_type())
    : super(from_comp, to_comp, al) {
    try {
      ...
      typename super::to::iterator thint = this->to.end();
      while (first != last) {
        std::pair<typename super::from::iterator, typename super::to::iterator> p = this->insert(fhint, thint, first++);
        fhint = p.first;
        thint = p.second;
      }
#endif
