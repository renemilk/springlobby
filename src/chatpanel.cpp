...
static wxColor m_irc_colors[16] = {
    wxColor(204, 204, 204), wxColor(0, 0, 0),      wxColor(54, 54, 178),  wxColor(42, 140, 42),
    wxColor(195, 59, 59),   wxColor(199, 50, 50),  wxColor(128, 38, 127), wxColor(102, 54, 31),
    wxColor(217, 166, 65),  wxColor(61, 204, 61),  wxColor(26, 85, 85),   wxColor(47, 140, 116),
    wxColor(69, 69, 230),   wxColor(176, 55, 176), wxColor(76, 76, 76),   wxColor(149, 149, 149)};
...
void ChatPanel::CreateControls() {
  ...
  if (m_show_nick_list) {
    ...
    // m_nick_filter = new wxComboBox( m_nick_panel, -1, _("Show all"), wxDefaultPosition, wxSize(80,CONTROL_HEIGHT), 0,
    // 0, wxCB_READONLY );
  ...
  if (m_type == CPT_Channel) {
    ...
    // m_chan_opts_button = new wxBitmapButton(m_chat_panel, CHAT_CHAN_OPTS,
    // icons().GetBitmap(icons().ICON_CHANNEL_OPTIONS), wxDefaultPosition , wxSize( CONTROL_HEIGHT, CONTROL_HEIGHT ) );
  } else if (m_type == CPT_User) {
    if (m_user) {
      // m_chan_opts_button = new wxBitmapButton(m_chat_panel, CHAT_CHAN_OPTS,
      // icons().GetBitmap(icons().GetUserBattleStateIcon(m_user->GetStatus()) ), wxDefaultPosition , wxSize(
  ...
  m_say_text = new wxTextCtrlHist(textcompletiondatabase, m_chat_panel, CHAT_TEXT, _T( "" ), wxDefaultPosition,
                                  wxSize(100, CONTROL_HEIGHT), wxTE_PROCESS_ENTER | wxTE_PROCESS_TAB);
...
const User& ChatPanel::GetMe() const { return serverSelector().GetServer().GetMe(); }
...
void ChatPanel::OutputLine(const ChatLine& line) {
  ...
  if (original_pos > 1.0f)
    original_pos = 1.0f; // windows isn't 100% right because thumb always returns 0
...
void ChatPanel::Said(const wxString& who, const wxString& message) {
  ...
  if ((who == _T( "MelBot" ) || who == _T( "[BOT]tizbacbridgebot" )) && message.StartsWith(_T( "<" )) &&
      message.Find(_T( ">" )) != wxNOT_FOUND) {
    wxString who2;
    wxString message2;
    who2 = message.BeforeFirst('>').AfterFirst('<');
    if (who != _T( "[BOT]tizbacbridgebot" ))
      who2 += _T( "@IRC" );
  ...
  if (req_user) {
    ...
    if (inactive) {
      ...
      UiEvents::GetNotificationEventSender().SendEvent(UiEvents::NotficationData(UiEvents::PrivateMessage, msg));
    }
  }
}
...
void ChatPanel::Joined(User& who) {
  if (m_type == CPT_Channel) {
    if (sett().GetDisplayJoinLeave(m_channel->GetName())) {
      ...
      OutputLine(_T( " ** " ) + who.GetNick() + _(" joined the ") + GetChatTypeStr() + _T( "." ),
                 sett().GetChatColorJoinPart(), sett().GetChatFont());
    }
  } else if (m_type == CPT_Battle) {
...
void ChatPanel::Parted(User& who, const wxString& message) {
  ...
  if (m_type == CPT_Channel) {
    ...
    if (sett().GetDisplayJoinLeave(m_channel->GetName())) {
      // change the image of the tab to show new events
      SetIconHighlight(highlight_join_leave);
      OutputLine(_T( " ** " ) + who.GetNick() + _(" left the ") + GetChatTypeStr() + _T( "( " ) + message + _T( " )." ),
                 sett().GetChatColorJoinPart(), sett().GetChatFont());
    }
  } else if (m_type == CPT_Battle) {
    if (sett().GetDisplayJoinLeave(_T( "game/battle" ))) {
...
void ChatPanel::SetTopic(const wxString& who, const wxString& message) {
 ...
 while ( pos != -1 )
 {
   ...
   refined = refined.Left ( pos -1 ) + _T("\n") + refined.Right( pos +1 ); // replace the /n string with the carriage
 return char
   pos = refined.Find( _T("\\n") );
 }
 */
...
void ChatPanel::UserStatusUpdated(User& who) {
  if ((m_type == CPT_User) && (m_user == &who) && (m_chan_opts_button != 0)) {
    m_chan_opts_button->SetBitmapLabel(
        icons().GetBitmap(icons().GetUserListStateIcon(who.GetStatus(), false, who.GetBattle() != 0)));
  }
  if (!m_show_nick_list || (m_nicklist == 0))
    return;
  try {
...
const Channel* ChatPanel::GetChannel() const { return m_channel; }
...
const Server* ChatPanel::GetServer() const { return m_server; }
...
const User* ChatPanel::GetUser() const { return m_user; }
...
void ChatPanel::SetUser(const User* usr) {
  ...
  if (m_user) {
    if (m_chan_opts_button != 0)
      m_chan_opts_button->SetBitmapLabel(
          icons().GetBitmap(icons().GetUserListStateIcon(m_user->GetStatus(), false, m_user->GetBattle() != 0)));
  }
  //	if ( m_user )
  //        m_chat_log.SetTarget( sett().GetDefaultServer(), usr->GetNick() );
}

bool ChatPanel::IsServerPanel() const { return (m_type == CPT_Server); }

ChatPanelType ChatPanel::GetPanelType() const { return m_type; }
  ...
  while (lines.HasMoreTokens()) {
    ...
    if (m_type == CPT_Channel) {
      ...
      if (line.StartsWith(_T( "/" ))) {
        if (m_channel->ExecuteSayCommand(line))
          return true;
        if (m_channel->GetServer().ExecuteSayCommand(line))
          return true;
        OutputLine(
            wxFormat(_(" Error: Command (%s) does not exist, use /help for a list of available commands.")) % line,
            sett().GetChatColorError(), sett().GetChatFont());
        return true;
      }
      m_channel->Say(line);
      ...
      if (m_battle == 0) {
        OutputLine(_(" You are not in battle or battle does not exist, use /help for a list of available commands."),
                   sett().GetChatColorError(), sett().GetChatFont());
        return true;
      }
      if (line.StartsWith(_T( "/" ))) {
        if (m_battle->ExecuteSayCommand(line))
          return true;
        if (m_battle->GetServer().ExecuteSayCommand(line))
          return true;
        OutputLine(
            wxFormat(_(" Error: Command (%s) does not exist, use /help for a list of available commands.")) % line,
            sett().GetChatColorError(), sett().GetChatFont());
      ...
      if (line.StartsWith(_T( "/" ))) {
        if (m_user->ExecuteSayCommand(line))
          return true;
        if (m_user->GetServer().ExecuteSayCommand(line))
          return true;
        OutputLine(
            wxFormat(_(" Error: Command (%s) does not exist, use /help for a list of available commands.")) % line,
            sett().GetChatColorError(), sett().GetChatFont());
      ...
      if (line.StartsWith(_T( "/" ))) {
        if (m_server->ExecuteSayCommand(line))
          return true;
        OutputLine(
            wxFormat(_(" Error: Command (%s) does not exist, use /help for a list of available commands.")) % line,
            sett().GetChatColorError(), sett().GetChatFont());
...
void ChatPanel::FocusInputBox() { m_say_text->SetFocus(); }
...
wxString ChatPanel::FindUrl(const long pos) const {
  ...
  while (end < last && m_chatlog_text->GetRange(end, end + 1) != _T(" ") &&
         m_chatlog_text->GetRange(end, end + 1) != _T("["))
    end++;
...
void ChatPanel::OnMenuItem(wxCommandEvent& event) { m_popup_menu->OnMenuItem(event); }
