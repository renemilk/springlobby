...
SinglePlayerBattle::SinglePlayerBattle(MainSinglePlayerTab& msptab)
  ...
  CustomBattleOptions().setSingleOption("startpostype", LSL::Util::ToString(ST_Pick),
                                        LSL::OptionsWrapper::EngineOption);
  ConnectGlobalEvent(this, GlobalEvent::OnUnitsyncReloaded,
                     wxObjectEventFunction(&SinglePlayerBattle::OnUnitsyncReloaded));
}
...
void SinglePlayerBattle::RemoveUnfittingBots() {
  ...
  LSL::StringVector::iterator end =
      std::set_difference(old_ais.begin(), old_ais.end(), new_ais.begin(), new_ais.end(), diff.begin());
...
void SinglePlayerBattle::Update(const wxString& Tag) { m_sptab.UpdateTag(Tag); }

void SinglePlayerBattle::StartSpring() { spring().Run(*this); }
