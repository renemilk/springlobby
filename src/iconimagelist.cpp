...
IconImageList::IconImageList() : wxImageList(16, 16, true) {
  ...
  ICON_STARTED_GAME_LOCKED =
      Add(charArr2wxBitmapWithBlending(closed_game_png, sizeof(closed_game_png), ingame_png, sizeof(ingame_png)));
...
int IconImageList::GetFlagIcon(const wxString& flagname) const { return ICON_FLAGS_BASE + GetFlagIndex(flagname); }
...
wxString getColourIconKey(const wxColour& colour) { return colour.GetAsString(wxC2S_HTML_SYNTAX).AfterFirst('#'); }
...
int IconImageList::GetHostIcon(const bool& spectator) const { return spectator ? ICON_HOST_SPECTATOR : ICON_HOST; }
...
int IconImageList::GetSideIcon(const wxString& modname, int side) {
  ...
  if (m_cached_side_icons.find(cachestring) == m_cached_side_icons.end()) {
    try {
      const auto img = LSL::usync().GetSidePicture(STD_STRING(modname), STD_STRING(sidename));
