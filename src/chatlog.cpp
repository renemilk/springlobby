...
bool ChatLog::OpenLogFile() {
  ...
  if (LogEnabled() && CreateCurrentLogFolder()) {
    ...
    if (!m_logfile.IsOpened()) {
      ...
      customMessageBox(SL_MAIN_ICON, _("Can't open log file \"") + logFilePath +
                                         _("\".\nBe sure that there isn't a write protection.\n"),
                       _("Log Warning"));
      m_active = false;
    } else {
      ...
      wxDateTime now = wxDateTime::Now();
      wxString text =
          _T( "### Session Start at [" ) + now.Format(_T( "%Y-%m-%d %H:%M" )) + _T( "]" ) + wxTextBuffer::GetEOL();
...
void ChatLog::OpenInEditor() { ui().OpenFileInEditor(GetCurrentLogfilePath()); }

const wxArrayString& ChatLog::GetLastLines() const { return m_last_lines; }

wxString ChatLog::GetCurrentLogfilePath() const {
  return sett().GetChatLogLoc() + wxFileName::GetPathSeparator() + m_server + wxFileName::GetPathSeparator() + m_room +
         _T( ".txt" );
}
 ...
 */
static inline ssize_t pread(int fd, void* buffer, size_t size, off_t offset) {
  errno = 0;
  if (offset < 0) {
    errno = EINVAL;
    return -1;
 ...
 */
static inline size_t find_tail_sequences(int fd, const char* bytes, size_t bytes_length, size_t count,
                                         wxArrayString& out) {
  ...
  while (count_added < count && last_read_position > 0) {
    ...
    size_t bytes_read(
        pread(fd, buf, std::min(static_cast<off_t>(read_size), last_read_position - read_position), read_position));
        ...
        if (have_last_pos && count_added < count) {
          ...
          if (line_length > 0) {
            ...
            if (strncmp(source, "##", 2) != 0) {
              ...
              wxLogMessage(_T("ChatLog::find_tail_sequences: fetching write buffer for %lu bytes"),
                           sizeof(wxChar) * (line_length + 1));
...
void ChatLog::FillLastLineArray() {
  ...
  wxLogMessage(_T("ChatLog::FillLastLineArray: Loaded %lu lines from %s."), lines_added,
               GetCurrentLogfilePath().c_str());
  close(fd);
