...
wxColour GetColorFromFloatStrng(const wxString& color_) {
  ...
  for (size_t i = 0; i < 3; ++i) {
    ...
    decimal_colors[i] = LSL::Util::Clamp(static_cast<unsigned char>(float_color * 256), static_cast<unsigned char>(0),
...
wxBitmap charArr2wxBitmap(const unsigned char* arg, int size) { return wxBitmap(charArr2wxImage(arg, size)); }
...
wxBitmap charArr2wxBitmapWithBlending(const unsigned char* dest, int dest_size, const unsigned char* text,
                                      int text_size) {
...
std::vector<wxColour>& GetBigFixColoursPalette(int numteams) {
  ...
  for (int i = result.size(); i < numteams; i++) {
    ...
    double saturation = 1;
    double value = 1;
    int switccolors = i % 3; // why only 3 and not all combinations? because it's easy, plus the bisection limit cannot
                             // be divided integer by it
    ...
    hue += 0.17; // use as starting point a zone where color band is narrow so that small variations means high change
                 // in visual effect
    if (hue > 1)
      hue -= 1;
...
wxColour GetColourFromUser(wxWindow* parent, const wxColour& colInit, const wxString& caption,
...
void OpenWebBrowser(const wxString& url) {
  if (sett().GetWebBrowserUseDefault()
    ...
    if (!wxLaunchDefaultBrowser(url)) {
      ...
      customMessageBoxNoModal(SL_MAIN_ICON, _("Couldn't launch browser. URL is: ") + url,
                              _("Couldn't launch browser."));
    }
  } else {
