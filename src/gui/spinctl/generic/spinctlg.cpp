...
public:
  SlSpinCtrlButtonGeneric(SlSpinCtrlGenericBase* spin, int style)
    : wxSpinButton(spin->GetParent(), wxID_ANY, wxDefaultPosition, wxDefaultSize, style | wxSP_VERTICAL) {
    m_spin = spin;
...
bool SlSpinCtrlGenericBase::Create(wxWindow* parent, wxWindowID id, const wxString& value, const wxPoint& pos,
                                   const wxSize& size, long style, double min, double max, double initial,
                                   double increment, const wxString& name) {
  ...
  if (!wxControl::Create(parent, id, wxDefaultPosition, wxDefaultSize, (style & ~wxBORDER_MASK) | wxBORDER_NONE,
...
wxSize SlSpinCtrlGenericBase::DoGetBestSize() const {
  wxSize sizeBtn = m_spinButton->GetBestSize(), sizeText = m_textCtrl->GetBestSize();
...
void SlSpinCtrlGenericBase::DoSetIncrement(double inc) { m_increment = inc; }
