...
void* SearchThread::Entry() {
  ...
  const wxString query =
      wxFormat(_("/json.php?nosensitive=on&logical=or&springname=%s&tag=%s")) % searchescaped % searchescaped;
...
ContentDownloadDialog::ContentDownloadDialog(wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos,
                                             const wxSize& size, long int style, const wxString& name)
...
bool ContentDownloadDialog::Show(bool show) { return wxDialog::Show(show); }
...
void ContentDownloadDialog::OnCloseButton(wxCommandEvent& /*event*/) { Close(); }
