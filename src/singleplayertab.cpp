...
SinglePlayerTab::SinglePlayerTab(wxWindow* parent, MainSinglePlayerTab& msptab)
  ...
  m_select_btn = new wxButton(this, SP_BROWSE_MAP, _T("..."), wxDefaultPosition, wxSize(CONTROL_HEIGHT, CONTROL_HEIGHT),
                              wxBU_EXACTFIT);
  ...
  m_color_btn =
      new ColorButton(this, SP_COLOUR, sett().GetBattleLastColour(), wxDefaultPosition, wxSize(30, CONTROL_HEIGHT));
...
void SinglePlayerTab::UpdateMinimap() { m_minimap->UpdateMinimap(); }
...
void SinglePlayerTab::ReloadModlist() {
  ...
  if (!m_battle.GetHostModName().IsEmpty()) {
    ...
    if (m_mod_pick->GetStringSelection() == wxEmptyString)
...
void SinglePlayerTab::ResetUsername() { m_battle.GetMe().SetNick(TowxString(LSL::usync().GetDefaultNick())); }
...
bool SinglePlayerTab::ValidSetup() const {
  ...
  if (m_battle.GetNumUsers() == 1) {
    ...
    if (customMessageBox(SL_MAIN_ICON,
                         _("Continue without adding a bot first?.\n The game will be over pretty fast.\n "),
...
void SinglePlayerTab::OnModSelect(wxCommandEvent& /*unused*/) {
  ...
  if (num_bots != m_battle.GetNumBots())
    customMessageBoxNoModal(SL_MAIN_ICON, _("Incompatible bots have been removed after game selection changed."),
                            _("Bots removed"));
}
...
void SinglePlayerTab::OnStart(wxCommandEvent& /*unused*/) {
  ...
  if (ui().IsSpringRunning()) {
    ...
    customMessageBoxNoModal(SL_MAIN_ICON, _("You cannot start a spring instance while another is already running"),
...
void SinglePlayerTab::OnRandomCheck(wxCommandEvent& /*unused*/) {
  if (m_random_check->IsChecked())
    m_battle.CustomBattleOptions().setSingleOption("startpostype", LSL::Util::ToString(IBattle::ST_Random),
                                                   LSL::OptionsWrapper::EngineOption);
  else
    m_battle.CustomBattleOptions().setSingleOption("startpostype", LSL::Util::ToString(IBattle::ST_Pick),
                                                   LSL::OptionsWrapper::EngineOption);
