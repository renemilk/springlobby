...
SpringLobbyApp::~SpringLobbyApp() { delete m_timer; }
...
bool SpringLobbyApp::OnInit() {
  ...
  wxLogWindow* loggerwin = InitializeLoggingTargets(0, m_log_console, m_log_file_path, m_log_window_show,
                                                    !m_crash_handle_disable, m_log_verbosity, logchain);
...
#ifdef __WXMSW__
  wxString path =
      wxPathOnly(wxStandardPaths::Get().GetExecutablePath()) + wxFileName::GetPathSeparator() + _T("locale");
#else
  ...
  if (!m_customizer_archive_name.IsEmpty()) { // this needsto happen before usync load
  ...
  if (!sett().IsFirstRun() && !usync_loaded) {
    customMessageBox(
        SL_MAIN_ICON,
        _("Please check that the file given in Preferences->Spring is a proper, readable unitsync library"),
  ...
  if (!m_customizer_archive_name.IsEmpty()) {
    if (SLcustomizations().Init(m_customizer_archive_name)) {
      ...
      customMessageBox(SL_MAIN_ICON, _("Couldn't load customizations for ") + m_customizer_archive_name +
      ...
      // that that is the correct name, passed in qoutation"), _("Fatal error") );
...
#define CSTR .c_str()
  ...
  static const wxCmdLineEntryDesc cmdLineDesc[] = {
      {wxCMD_LINE_SWITCH,                     STR("h"),            STR("help"),
       wxTRANSLATE("show this help message"), wxCMD_LINE_VAL_NONE, wxCMD_LINE_OPTION_HELP},
      {wxCMD_LINE_SWITCH,       STR("nc"),
       STR("no-crash-handler"), wxTRANSLATE("don't use the crash handler (useful for debugging)"),
       wxCMD_LINE_VAL_NONE,     wxCMD_LINE_PARAM_OPTIONAL},
      {wxCMD_LINE_OPTION,     STR("fl"),
       STR("file-logging"),   wxTRANSLATE("dumps application log to a file ( enter path )"),
       wxCMD_LINE_VAL_STRING, wxCMD_LINE_PARAM_OPTIONAL | wxCMD_LINE_NEEDS_SEPARATOR},
      {wxCMD_LINE_SWITCH,      STR("cl"),
       STR("console-logging"), wxTRANSLATE("shows application log to the console(if available)"),
       wxCMD_LINE_VAL_NONE,    wxCMD_LINE_PARAM_OPTIONAL},
      {wxCMD_LINE_SWITCH,                             STR("gl"),           STR("gui-logging"),
       wxTRANSLATE("enables application log window"), wxCMD_LINE_VAL_NONE, wxCMD_LINE_PARAM_OPTIONAL},
      {wxCMD_LINE_OPTION,     STR("f"),
       STR("config-file"),    wxTRANSLATE("override default choice for config-file"),
       wxCMD_LINE_VAL_STRING, wxCMD_LINE_PARAM_OPTIONAL | wxCMD_LINE_NEEDS_SEPARATOR},
      {wxCMD_LINE_OPTION, STR("l"), STR("log-verbosity"),
       wxTRANSLATE("overrides default logging verbosity, can be:\n                                0: no log\n          "
                   "                      1: critical errors\n                                2: errors\n              "
                   "                  3: warnings (default)\n                                4: messages\n             "
                   "                   5: function trace"),
       wxCMD_LINE_VAL_NUMBER, wxCMD_LINE_PARAM_OPTIONAL},
      {wxCMD_LINE_OPTION,     STR("c"),
       STR("customize"),      wxTRANSLATE("load lobby customizations from game archive. Expects the shortname."),
       wxCMD_LINE_VAL_STRING, wxCMD_LINE_PARAM_OPTIONAL},
      {wxCMD_LINE_OPTION,                                 STR("n"),              STR("name"),
       wxTRANSLATE("overrides default application name"), wxCMD_LINE_VAL_STRING, wxCMD_LINE_PARAM_OPTIONAL},
      {wxCMD_LINE_NONE, NULL,                NULL,
       NULL,            wxCMD_LINE_VAL_NONE, wxCMD_LINE_PARAM_OPTIONAL} // this is mandatory according to
      // http://docs.wxwidgets.org/stable/wx_wxcmdlineparser.html
  };
