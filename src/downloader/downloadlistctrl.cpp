...
template <>
SortOrder DownloadListCtrl::BaseType::m_sortorder = SortOrder();
...
void DownloadListCtrl::SetTipWindowText(const long /*item_hit*/, const wxPoint& /*position*/) { m_tiptext = _T(""); }
...
int DownloadListCtrl::CompareOneCrit(DataType u1, DataType u2, int col, int dir) const {
  switch (col) {
    case 0:
      return dir * compareSimple(boost::to_lower_copy(std::string(u1.name.mb_str())),
                                 boost::to_lower_copy(std::string(u2.name.mb_str())));
    ...
    case 3:
      return dir * compareSimple(u1.progress, u2.progress);
...
int DownloadListCtrl::GetItemColumnImage(long /*item*/, long /*column*/) const { return -1; }

int DownloadListCtrl::GetItemImage(long /*item*/) const { return -1; }
  ...
  switch (column) {
    ...
    case 2:
      return wxString::Format(
          wxT("%i%%"), (int)((double)100.0 * infos.progress /
                             (double)infos.size)); //(wxFormat(_T("%.2f") ) % ( infos.progress * 100 )).str() : na_str;
    case 3:
      return _T(""); // infos.inspeed > -0.01 ? (wxFormat(_T("%.2f") ) % ( infos.inspeed*kfactor )).str() : na_str;
    case 4:
      return _T(""); // infos.inspeed > -0.01 ? (wxFormat(_T("%.2f") ) % ( infos.inspeed*kfactor )).str() : na_str;
    case 5:
      return infos.size > 0 ? wxString::Format(wxT("%i"), infos.size / MB) : _T("0");
  }
}
...
bool DownloadListCtrl::IsTorrentActive(const DataType& info) const { return !info.finished; }
