...
SLNotebook::SLNotebook(wxWindow* parent, const wxString& name, wxWindowID id, const wxPoint& pos, const wxSize& size,
                       long style)
  : wxAuiNotebook(parent, id, pos, size, style) {
  SetName(name);
  m_mgr.SetFlags(wxAUI_MGR_ALLOW_FLOATING | wxAUI_MGR_HINT_FADE | wxAUI_MGR_VENETIAN_BLINDS_HINT);
}
...
SLChatNotebook::SLChatNotebook(wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long style)
  : ParentType(parent, _T("chatnotebook"), id, pos, size,
               sett().GetShowXallTabs() ? style | wxAUI_NB_CLOSE_ON_ALL_TABS : style)
  , m_ch_menu(0)
  , m_cur_page(0) {}

// should work accroding to docs/sample, but doesn't (not even in sample)
void SLChatNotebook::OnAllowNotebookDnD(wxAuiNotebookEvent& evt) { evt.Veto(); }
...
bool SLChatNotebook::DeleteChatPage(size_t i) {
  ...
  // the checking for server panel is not supposed to be here, but it prevents a nasty crash bug in handling 'close all'
  // for the time being
...
public:
  ...
  ~wxTabFrame() { wxDELETE(m_tabs); }

  void SetTabCtrlHeight(int h) { m_tab_ctrl_height = h; }

  void DoSetSize(int x, int y, int width, int height, int WXUNUSED(sizeFlags = wxSIZE_AUTO)) {
...
#endif
    new_tabs->m_tabs = new wxAuiTabCtrl(this, nextid, wxDefaultPosition, wxDefaultSize, wxNO_BORDER | wxWANTS_CHARS);
