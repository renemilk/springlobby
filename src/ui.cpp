...
ChatPanel* Ui::GetActiveChatPanel() { return mw().GetActiveChatPanel(); }
...
void Ui::Reconnect() {
  ...
  if (!sett().GetServerAccountSavePass(servname)) {
    if (!AskPassword(_("Server password"), _("Password"), pass))
      return;
  }
...
void Ui::DoConnect(const wxString& servername, const wxString& username, const wxString& password) {
  ...
  if (sett().GetServerAccountSavePass(servername)) {
    if (serverSelector().GetServer().IsPasswordHash(password))
      sett().SetServerAccountPass(servername, password);
    else
      sett().SetServerAccountPass(servername, serverSelector().GetServer().GetPasswordHash(password));
  } else {
...
bool Ui::DoRegister(const wxString& servername, const wxString& username, const wxString& password, wxString& reason) {
  ...
  if (success) {
    customMessageBox(SL_MAIN_ICON, _("Registration successful,\nyou should now be able to login."),
                     _("Registration successful"), wxOK);
  } else {
...
bool Ui::IsConnecting() const {
  if (m_reconnect_dialog != 0)
    return m_reconnect_dialog->IsShown();
  return false;
}
...
bool Ui::IsSpringRunning() const { return spring().IsRunning(); }
...
//	wxString url =
//_T("http://spring.jobjol.nl/search_result.php?search_cat=1&select_select=select_file_subject&Submit=Search&search=") +
// newname;
//	OpenWebBrowser ( url );
//}
...
void Ui::ConsoleHelp(const wxString& topic) {
  ...
  if (panel == 0) {
    ShowMessage(_("Help error"),
                _("Type /help in a chat box. (A bug currently prevents this from working in battleroom"));
    return;
  }
  if (topic == wxEmptyString) {
    ...
    panel->ClientMessage(_("  \"/changepassword2 newpassword\" - Changes the current active account's password, needs "
                           "the old password saved in login box"));
    panel->ClientMessage(_("  \"/changepassword oldpassword newpassword\" - Changes the current active account's "
                           "password, password cannot contain spaces"));
    panel->ClientMessage(_("  \"/channels\" - Lists currently active channels."));
    panel->ClientMessage(
        _("  \"/help [topic]\" - Put topic if you want to know more specific information about a command."));
    ...
    panel->ClientMessage(_("The topic \"") + topic +
                         _("\" was not found. Type \"/help topics\" only for available topics."));
  }
}

ChatPanel* Ui::GetChannelChatPanel(const wxString& channel) { return mw().GetChannelChatPanel(channel); }
...
bool Ui::IsSpringCompatible(const wxString& engine, const wxString& version) {
  ...
  if (versionlist.size() == 0) {
    ...
    customMessageBoxNoModal(
        SL_MAIN_ICON,
        _("Couldn't get your spring versions from any unitsync library.\n\nOnline play is currently disabled."),
        _("Spring error"), wxICON_EXCLAMATION | wxOK);
    return false;
  }
  for (const auto pair : versionlist) {
    if ((STD_STRING(pair.second) == neededversion) || (STD_STRING(pair.second) == hackversion)) {
      if (sett().GetCurrentUsedSpringIndex() != pair.first) {
        wxLogMessage(_T("server enforce usage of version: %s, switching to profile: %s"), neededversion.c_str(),
                     pair.first.c_str());
  ...
  wxString message =
      wxFormat(_("No compatible installed spring version has been found, this server requires version: %s\n")) %
      neededversion;
...
void Ui::OnDisconnected(Server& server, bool wasonline) {
  ...
  if ((!m_disable_autoconnect) &&
      (!wasonline)) { // couldn't even estabilish a socket, prompt the user to switch to another server
    ConnectionFailurePrompt();
  }
}
...
wxString Ui::GetNextServer() {
  ...
  if (previous_server.IsEmpty())
    previous_server = sett().GetDefaultServer();
...
static inline bool IsAutoJoinChannel(Channel& chan) {
  typedef std::vector<ChannelJoinInfo> Vec;
  typedef Vec::const_iterator VecIt;
...
void Ui::OnUserSaid(User& user, const wxString& message, bool fromme) {
  ...
  if (fromme)
    user.uidata.panel->Said(serverSelector().GetServer().GetMe().GetNick(), message);
  else
    user.uidata.panel->Said(user.GetNick(), message);
}
...
void Ui::OnUserSaidEx(User& user, const wxString& action, bool fromme) {
  ...
  if (fromme)
    user.uidata.panel->DidAction(serverSelector().GetServer().GetMe().GetNick(), action);
  else
    user.uidata.panel->DidAction(user.GetNick(), action);
}
...
void Ui::OnUserLeftBattle(IBattle& battle, User& user, bool isbot) {
  ...
  user.BattleStatus().side =
      0; // and reset side, so after rejoin we don't potentially stick with a num higher than avail
  mw().GetBattleListTab().UpdateBattle(battle);
  try {
    if (mw().GetJoinTab().GetBattleRoomTab().GetBattle() == &battle) {
...
void Ui::OnJoinedBattle(Battle& battle) {
  ...
  if (!LSL::usync().IsLoaded()) {
    customMessageBox(SL_MAIN_ICON, _("Your spring settings are probably not configured correctly,\nyou should take "
                                     "another look at your settings before trying\nto play online."),
                     _("Spring settings error"), wxOK);
  }
...
//                    if(eta <= 0) {
//						serverSelector().GetServer().GetCurrentBattle()->ExecuteSayCommand(_("/me map is not available for
...
// //there is no direct from-web downloading in SL currently
...
bool Ui::IsThisMe(User& other) const { return IsThisMe(other.GetNick()); }

bool Ui::IsThisMe(User* other) const { return ((other != 0) && IsThisMe(other->GetNick())); }
...
int Ui::TestHostPort(unsigned int port) const { return serverSelector().GetServer().TestOpenPort(port); }
...
void Ui::OpenFileInEditor(const wxString& filepath) {
  ...
  if (editor_path == wxEmptyString) {
    customMessageBoxNoModal(
        SL_MAIN_ICON, _T("You have not chosen an external text editor to open files with.\nPlease Select one now."),
        _T("No editor set"));
  ...
  if (!success) {
    customMessageBoxNoModal(SL_MAIN_ICON, _T("There was a problem launching the editor.\nPlease make sure the path is ")
                                          _T("correct and the binary executable for your user.\nNote it's currently ")
                                          _T("not possible to use shell-only editors like ed, vi, etc."),
                            _T("Problem launching editor"));
    mw().ShowConfigure(MainWindow::OPT_PAGE_GENERAL);
  }
}
...
void Ui::FirstRunWelcome() {
  if (sett().IsFirstRun()) {
    ...
    wxMessageBox(
        wxFormat(_("Hi %s,\nIt looks like this is your first time using %s. I have guessed a configuration that I "
                   "think will work for you but you should review it, especially the Spring configuration.")) %
            wxGetUserName() % GetAppName(),
        _("Welcome"), wxOK | wxICON_INFORMATION, &mw());
    ...
    if (!uikeyslocation.IsEmpty()) {
      wxCopyFile(uikeyslocation, sett().GetCurrentUsedDataDir() + wxFileName::GetPathSeparator() + _T("uikeys.txt"),
                 false);
    }
...
void Ui::CheckForUpdates() {
  ...
  if (latestVersion == _T("-1")) {
    customMessageBoxNoModal(SL_MAIN_ICON,
                            _("There was an error checking for the latest version.\nPlease try again later.\nIf the "
                              "problem persists, please use Help->Report Bug to report this bug."),
                            _("Error"));
    return;
  }
...
#ifdef __WXMSW__
    int answer = customMessageBox(
        SL_MAIN_ICON, wxFormat(_("Your %s version is not up to date.\n\n%s\n\nWould you like for me to autodownload "
                                 "the new version? Changes will take effect next you launch the lobby again.")) %
                          GetAppName() % msg,
        _("Not up to date"), wxYES_NO);
    if (answer == wxYES) {
      wxString command = _T("\"") + wxPathOnly(wxStandardPaths::Get().GetExecutablePath()) +
                         wxFileName::GetPathSeparator() + _T("springlobby_updater.exe\"");
      wxString params =
          _T("-f \"") + wxStandardPaths::Get().GetExecutablePath() + _T("\"") + _T(" -r ") + latestVersion;
      if (WinExecute(command, params) > 0) {
        // returned pid > 0 -> proc started successfully
        // now close this instance immeadiately
        wxCloseEvent dummy;
        ui().mw().OnClose(dummy);
      } else { // this will also happen if updater exe is not present so we don't really ne special check for existance
               // of it
        customMessageBox(SL_MAIN_ICON, _("Automatic update failed\n\nyou will be redirected to a web page with "
                                         "instructions and the download link will be opened in your browser.") +
                                           msg,
                         _("Updater error."));
  ...
  /* TODO currently not usable cause automatic update calls this function too and we don't want a msg box everytime the
  check succeeds
  else
