...
Spring::~Spring() { delete m_process; }

bool Spring::IsRunning() const { return m_running; }
...
bool Spring::LaunchSpring(const wxString& params) {
  ...
  if (!wxFile::Exists(sett().GetCurrentUsedSpringBinary())) {
    customMessageBoxNoModal(SL_MAIN_ICON,
                            _T("The spring executable was not found at the set location, please re-check."),
                            _T("Executable not found"));
...
#ifdef __WXMSW__
    if (LSL::usync().GetSpringVersion().find("0.78.") != std::string::npos)
      configfileflags = _T("");
#endif
  }
  ...
  if (sett().GetCurrentUsedSpringBinary().AfterLast(_T('.')) == _T("app"))
    cmd += sep + wxString(_T("Contents")) + sep + wxString(_T("MacOS")) + sep +
           wxString(_T("spring")); // append app bundle inner path
#endif
...
wxString Spring::WriteScriptTxt(IBattle& battle) const {
  ...
  if (battle.IsFounderMe()) {
    ...
    if (battle.GetNatType() == NAT_Hole_punching) {
      ...
      tdf.Append(_T("SourcePort"), sett().GetClientPort()); /// this allows to play with broken router by setting
                                                            /// SourcePort to some forwarded port.
    }
  }
  ...
  **********************************************************************************/
  ...
  if (battle.IsProxy() && (startpostype != IBattle::ST_Pick) && (startpostype != IBattle::ST_Choose)) {
    ...
    remap_positions = std::vector<LSL::StartPos>(infos.positions.begin(),
                                                 infos.positions.begin() + copysize); // only add the first x positions
  ...
  if (LSL::usync().VersionSupports(LSL::USYNC_GetSkirmishAI)) {
    for (unsigned int i = 0; i < NumUsers; i++) {
      ...
      if (optionmapindex > 0) {
        for (const auto& it :
             battle.CustomBattleOptions().getOptions((LSL::OptionsWrapper::GameOption)optionmapindex)) {
          tdf.Append(TowxString(it.first), TowxString(it.second.second));
        }
      }
      tdf.LeaveSection();
  ...
  for (unsigned int i = 0; i < NumUsers; i++) {
    ...
    wxString colourstring = TowxString(status.colour.Red() / 255.0) + _T(' ') +
