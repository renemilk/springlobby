...
void FillAliasMap() {
  ...
  for (std::map<wxString, wxString>::const_iterator i = m_command_alias.begin(); i != m_command_alias.end(); ++i)
    m_send_command_alias[i->second] = i->first; // swap content for second map
}
...
wxString EncodeTokenMessage(const wxString& cmd) {
  ...
  if (it != m_send_command_alias.end())
    return it->second;
  else
    return cmd;
}
